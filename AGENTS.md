...
- **2026-09-16 (Shopee-Style Mobile Product Detail Page Architecture):**
  - **1. Dedicated Mobile Shopee Layout ([`shopee-mobile-product-detail.tsx`](file:///src/components/gadget/shopee-mobile-product-detail.tsx)):** Mengimplementasikan antarmuka detail produk mobile identik Shopee (`block lg:hidden`), meliputi floating app bar transparan/solid on scroll, galeri hero 1:1 edge-to-edge dengan badge slide counter (`1/4`), quick-selector varian thumbnail horizontal, bar harga bold dengan simulasi cicilan, badge Hadiah Gratis (bonus 3-in-1), bar estimasi logistik & garansi 30 hari, kartu toko fisik dengan 3 metrik statistik, carousel produk cabang lain, spesifikasi collapsible, deskripsi expandable, review buyer, rekomendasi 2 kolom, dan floating action bar bottom (`Chat Sekarang`, `+ Keranjang`, `Beli Sekarang`) dengan scroll-to-top button.
  - **2. Desktop Layout Preservation ([`page.tsx`](file:///src/app/gadget/[id]/page.tsx)):** Mempertahankan 100% tata letak bento desktop (`hidden lg:flex`) lengkap dengan breadcrumb, bento gallery, panel spesifikasi hardware, dan navbar/footer utama.
  - **3. Backend API Enrichment ([`route.ts`](file:///src/app/api/gadgets/[id]/route.ts)):** Menyertakan produk toko lain (`store.products`), total katalog toko (`store._count.products`), dan produk rekomendasi (`relatedProducts`) dalam response API detail gadget.
