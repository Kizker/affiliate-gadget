...
- **2026-09-25 (Modul Keamanan Penarikan Saldo — 3 Gerbang Keamanan, Cooling-down 24 Jam, Validasi Nama PT, 2-Step OTP, & Rate Limiting):**
  ...
  - **3. Validasi Kesesuaian Nama Pemilik Rekening vs Badan Hukum PT ([`withdrawal-security.ts`](file:///src/lib/withdrawal-security.ts), [`withdraw/route.ts`](file:///src/app/api/admin/finance/withdraw/route.ts)):** Normalisasi nama badan usaha (stripping prefix PT/CV/UD, tanda baca, simbol) dan perbandingan fuzzy similarity score (minimum 70%) dengan dukungan switch `WITHDRAWAL_NAME_VALIDATION_BYPASS` untuk environment dev/staging.
  - **4. Tantangan OTP 2FA Penarikan & Rate Limiting ([`withdraw/route.ts`](file:///src/app/api/admin/finance/withdraw/route.ts), [`send-otp/route.ts`](file:///src/app/api/auth/send-otp/route.ts)):** Rate limit ketat maksimal 3x OTP penarikan per 24 jam per user/toko, verifikasi OTP 6 digit hash bcrypt dengan konsumsi token sekali pakai, sliding window rate limiter maksimal 5 request penarikan per jam, audit log komprehensif, notifikasi email konfirmasi pencairan resmi, serta pengiriman email peringatan keamanan jika OTP salah 3 kali berturut-turut.
  - **5. Frontend UI Modal Penarikan 2 Tahap & Alert Cooling-down ([`finance/page.tsx`](file:///src/app/dashboard/admin/finance/page.tsx)):** Redesain modal penarikan menjadi state machine 3 tahap (Input Nominal -> Verifikasi OTP 2FA -> Konfirmasi Sukses). Banner peringatan cooling-down 24 jam reaktif di atas kartu Saldo Siap Cair dengan penguncian tombol otomatis, timer countdown 300s, cooldown kirim ulang 60s, email masking, dan penanganan badge error spesifik server.
  - **6. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), 60 unit test files dengan 491 unit tests lulus 100% (`pnpm test:unit`), mencakup test unit isolasi dan test integrasi Route Handler E2E ([`withdrawal-route-e2e.test.ts`](file:///tests/unit/withdrawal-route-e2e.test.ts)). Ditambah 7 skenario pengujian browser penuh Playwright Chromium E2E ([`tests/e2e/withdrawal.spec.ts`](file:///tests/e2e/withdrawal.spec.ts)) lulus 100% (Auth redirect, verifikasi rekening PT, validasi nominal modal, challenge OTP & countdown, modal error feedback cooling-down 24 jam, dialog konfirmasi resi sukses, dan dashboard locked cooling-down banner).

- **2026-09-25 (Store Admin & Dummy Accounts 2FA Bypass & Isolated Mock Engine):**
  - **1. Store & Dummy Account 2FA Exemption ([`api/auth/login-2fa/route.ts`](file:///src/app/api/auth/login-2fa/route.ts)):** Membebaskan akun toko cabang fisik (`STORE_ADMIN`), admin platform (`ADMIN`, `SUPER_ADMIN`), staf internal (`TECHNICIAN`, `STORE_SALES`, `FINANCE_ADMIN`), dan akun email dummy (`@affiliategadget.com`, `@test.com`) dari kewajiban verifikasi OTP saat login. Mereka langsung login via kredensial password tanpa terhalang inbox email dummy.
  - **2. Granular Notification Mock Switches ([`whatsapp-provider.ts`](file:///src/lib/notifications/whatsapp-provider.ts), [`sms-provider.ts`](file:///src/lib/notifications/sms-provider.ts), [`.env`](file:///c:/.PROJECT/affiliate-gadget/.env)):** Dukungan switch terpisah `WA_MOCK_MODE=true` dan `SMS_MOCK_MODE=true` sembari menjaga email tetap `REAL` via Resend, serta format output terminal mock OTP yang bersih dan informatif.
  - **3. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), 55 unit test files dengan 437 unit tests lulus 100% (`pnpm test:unit`).

- **2026-09-24 (Email-First OTP Verification Architecture, Multi-Channel Fallback & Dual-Flow Suite):**
  - **1. Privacy Email Masking Utility ([`utils.ts`](file:///src/lib/utils.ts)):** Fungsi pembantu `maskEmail()` untuk menyembunyikan identitas username email pada antarmuka publik (`bam***@gmail.com`) dengan proteksi input string pendek/tidak valid.
  - **2. Backend Email-First Routing & Multi-Channel Actions ([`api/auth/register/route.ts`](file:///src/app/api/auth/register/route.ts), [`api/auth/login-2fa/route.ts`](file:///src/app/api/auth/login-2fa/route.ts)):**
    - Alur registrasi akun: Default pengiriman OTP dialihkan ke `EMAIL` (bebas biaya saldo Zenziva), respon JSON menyertakan `maskedEmail`, `hasPhone`, dan `requiresOtp: true`.
    - Alur Login 2FA: Pengiriman default OTP saat `action: 'check'` ke `EMAIL`, penambahan aksi eksplisit `resend-email`, `resend-wa`, dan `resend-sms`.
    - Resolusi verifikasi berbasis `identifierUsed` untuk mencocokkan OTP DB baik jika pengguna tetap di email maupun beralih ke WhatsApp/SMS.
  - **3. User Interface & Multi-Channel Fallback UI ([`register/page.tsx`](file:///src/app/register/page.tsx), [`login/page.tsx`](file:///src/app/login/page.tsx)):**
    - Tampilan header kartu OTP reaktif per channel (`EMAIL` badge biru, `WHATSAPP` badge hijau, `SMS` badge kuning).
    - Tombol expandable _"Pilihan pengiriman lain"_ jika akun memiliki data nomor HP, memfasilitasi pengalihan pengiriman ke WhatsApp atau SMS secara instan.
  - **4. Unit Testing Suite ([`email-first-otp-flow.test.ts`](file:///tests/unit/email-first-otp-flow.test.ts), [`register-phone-otp-flow.test.ts`](file:///tests/unit/register-phone-otp-flow.test.ts)):** 14 unit tests memvalidasi masking email, alur login check email default, alur resend WA/SMS, verifikasi identifier ganda, dan registrasi email-first.
  - **5. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), 55 unit test files dengan 436 unit tests lulus 100% (`pnpm test:unit`).

- **2026-09-24 (Multi-Channel Notification System, Zenziva WA/SMS, Resend Email, Device Fingerprinting & OTP Security Suite):**
  - **1. Multi-Channel Notification Dispatcher ([`src/lib/notifications/`](file:///src/lib/notifications/)):** Abstraksi provider Zenziva (WhatsApp & SMS) dan Resend (Email), orkestrator terpadu `notification-dispatcher.ts` dengan isolasi error asinkron, template transaksional ramah seluler, audit trail otomatis pada tabel `NotificationLog`, dan dukungan switch `NOTIFICATION_MOCK_MODE`.
  - **2. Secure OTP Engine & Multi-Purpose Verification ([`otp-generator.ts`](file:///src/lib/notifications/otp-generator.ts), [`two-factor-store.ts`](file:///src/lib/two-factor-store.ts), [`api/auth/send-otp`](file:///src/app/api/auth/send-otp/route.ts), [`api/auth/verify-otp`](file:///src/app/api/auth/verify-otp/route.ts)):** Migrasi penuh dari file JSON ke tabel database PostgreSQL `OtpToken` dengan hashing bcrypt, rate limiting 5 request/hari, cooldown 60 detik, kedaluwarsa 300 detik, proteksi brute-force 3 kali percobaan, serta dukungan purpose (`LOGIN`, `REGISTER`, `CHANGE_PHONE`, `CHANGE_EMAIL`, `CHANGE_PASSWORD`, `TRANSACTION_CONFIRMATION`).
  - **3. Device Fingerprinting & Session Security ([`device-fingerprint.ts`](file:///src/lib/device-fingerprint.ts), [`auth.ts`](file:///src/auth.ts), [`security/devices`](file:///src/app/api/user/security/devices/route.ts), [`security/revoke-session`](file:///src/app/api/user/security/revoke-session/route.ts)):** Sidik jari deterministik SHA-256 berbasis User-Agent dan subnet IP, deteksi perangkat baru saat login dengan pengiriman instan peringatan keamanan via WA & Email, dashboard manajemen perangkat aktif di `/dashboard/customer/security`, dan pemutusan sesi darurat instan.
  - **4. User Interface & End-to-End Auth Flows ([`register/page.tsx`](file:///src/app/register/page.tsx), [`login/page.tsx`](file:///src/app/login/page.tsx), [`settings/page.tsx`](file:///src/app/dashboard/customer/settings/page.tsx)):**
    - Step OTP verifikasi HP inline saat registrasi lengkap dengan timer 300s, cooldown 60s, dan fallback SMS.
    - Modal challenge OTP saat penggantian nomor HP di profil customer dengan pencegahan nomor ganda dan pembaruan timestamp `phoneVerified`.
    - Alur login 2FA dengan opsi kirim ulang WhatsApp dan fallback SMS.
  - **5. Transactional Events Notification Integration:** Integrasi pengiriman WA & Email otomatis pada checkout (`ORDER_CREATED`), webhook Midtrans (`PAYMENT_VERIFIED`), penyerahan kurir & pembaruan status (`ORDER_SHIPPED`), webhook logistik kurir (`ORDER_DELIVERED`), dan konfirmasi penerimaan pesanan (`ORDER_COMPLETED`).
  - **6. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), 54 unit test files dengan 427 unit tests lulus 100% (`pnpm test:unit`).
...
- **2026-09-24 (Komponen 3: PPh 23 & PPN Dashboard Integration E2E - Standar Badan Usaha CV):**
  ...
  - **3. Store Admin Finance Dashboard Streamlining ([`finance/page.tsx`](file:///src/app/dashboard/admin/finance/page.tsx)):** Mempertahankan 4 kartu KPI operasional kas murni (Saldo Siap Cair, Pendapatan Kotor, Bagi Hasil Platform, Dana Tertahan Escrow) tanpa kartu catatan PPh Final/pajak di grid atas, serta menyediakan panel samping e-Billing PPh 23 dan tab mutasi PPh 23.
  - **4. Settings Page Live Preview ([`settings/page.tsx`](file:///src/app/dashboard/admin/settings/page.tsx)):** Menambahkan banner informasi KPP Pratama dan tabel simulasi kalkulasi PPh 23 & beban jasa komisi toko secara reaktif.
  - **5. Consolidated Excel Tax Summary Sheet ([`api/admin/reports/export/route.ts`](file:///src/app/api/admin/reports/export/route.ts)):** Menambahkan worksheet ke-2 `RINGKASAN_PAJAK` pada ekspor Excel finansial lengkap dengan formatting mata uang dan garis ganda akuntansi.
  - **6. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`).

- **2026-09-23 (Store Admin Finance Period Filter, Multi-Period Metrics & Excel Export Integration):**
  - **1. Multi-Period Date Filter Control ([`finance/page.tsx`](file:///src/app/dashboard/admin/finance/page.tsx)):** Integrasi kontrol dropdown filter periode yang identik dengan superadmin (Hari Ini, Minggu Ini, Bulan Ini, Tahun Ini, dan 12 Bulan Spesifik) pada halaman Keuangan Admin Toko.
  - **2. Real-Time Finance API Date Range Filtering ([`api/admin/finance/route.ts`](file:///src/app/api/admin/finance/route.ts)):** Ekstraksi parameter `startDate` dan `endDate` dengan guard NaN. Menjaga Saldo Siap Cair (`availableBalance`) tetap riil all-time berbasis `allTimeCompletedAgg` agar dompet pencairan toko tidak hilang, sembari menyaring omzet periode, komisi platform, PPN keluaran, dan mutasi kas sesuai rentang tanggal yang dipilih.
  - **3. Synchronized Excel Export Integration ([`finance/page.tsx`](file:///src/app/dashboard/admin/finance/page.tsx), [`api/admin/reports/export/route.ts`](file:///src/app/api/admin/reports/export/route.ts)):** Meneruskan `startDate` dan `endDate` aktif langsung ke endpoint ekspor Laporan Keuangan (`type=financials`) dan Laporan Pesanan (`type=orders`), menyertakan label periode dinamis pada nama file unduhan.
  - **4. Unit Testing Suite ([`store-finance-date-filter.test.ts`](file:///tests/unit/store-finance-date-filter.test.ts)):** 8 unit tests lolos 100% memvalidasi resolusi rentang tanggal, pembentukan query string ekspor, dan isolasi saldo riil all-time vs pendapatan periode.
  - **5. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), 41 unit test files dengan 346 unit tests lulus 100% (`pnpm test:unit`).

- **2026-09-23 (Financial Reports Currency Formatting, Multi-Store Export & UI Streamlining):**
  - **1. Real-Time Escrow & Superadmin Dashboard Metrics ([`api/admin/finance/route.ts`](file:///src/app/api/admin/finance/route.ts), [`api/admin/dashboard/route.ts`](file:///src/app/api/admin/dashboard/route.ts), [`finance/page.tsx`](file:///src/app/dashboard/admin/finance/page.tsx), [`page.tsx`](file:///src/app/dashboard/admin/page.tsx)):** Integrasi real-time status pesanan logistik (IN_PROGRESS, SHIPPED) untuk kalkulasi dana tertahan (escrow), penghapusan label statis deskriptif yang membingungkan, dan sinkronisasi real-time metrik superadmin.
  - **2. Standardized Excel Currency Formatting & Accounting Totals ([`reports-export-format.ts`](file:///src/lib/reports-export-format.ts), [`api/admin/reports/export/route.ts`](file:///src/app/api/admin/reports/export/route.ts)):** Penerapan format mata uang Rupiah konsisten (`"Rp "#,##0;[Red]("-Rp "#,##0);"Rp 0"`) pada setiap kolom numerik finansial di file Excel export, dilengkapi garis ganda akuntansi (_double bottom border_) pada baris ringkasan TOTAL.
  - **3. Dedicated Store Admin Financial Reports Export Access ([`finance/page.tsx`](file:///src/app/dashboard/admin/finance/page.tsx), [`sidebar.tsx`](file:///src/components/dashboard/sidebar.tsx)):** Menyediakan tombol export Laporan Keuangan dan Pesanan langsung pada halaman Keuangan Admin Toko yang terisolasi per `storeId`, serta penyederhanaan label tombol (menghilangkan suffix "(Excel)").
  - **4. Financial Reports Page Streamlining ([`reports/page.tsx`](file:///src/app/dashboard/admin/reports/page.tsx)):** Menghapus bagian yang tidak esensial dari tab laporan finansial Superadmin (_Komposisi Penjualan_, _Distribusi Status Order_, dan _Garansi & Layanan Servis Kilat LCD_), serta membersihkan dependensi `chart.js` dan icon `lucide-react` yang tidak lagi digunakan.
  - **5. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), 36 unit test files dengan 297 unit tests lulus 100% (`pnpm test:unit`).

- **2026-09-22 (Admin Products HPP Input & Financial Calculations UI Integration):**
  - **1. Form Input HPP Produk & Varian ([`products/new/page.tsx`](file:///src/app/dashboard/admin/products/new/page.tsx) & [`products/[id]/edit/page.tsx`](file:///src/app/dashboard/admin/products/[id]/edit/page.tsx)):** Menambahkan field input _"Harga Modal / HPP (Rp)"_ di samping Harga Jual dan Harga Coret pada form produk utama, serta kolom _"HPP Modal (Rp)"_ pada tabel varian (RAM/Storage/Warna).
  - **2. Product Management APIs Enhancement ([`api/products/route.ts`](file:///src/app/api/products/route.ts) & [`api/products/[id]/route.ts`](file:///src/app/api/products/[id]/route.ts)):** Mendukung ekstraksi dan persistensi `costPrice` pada level produk dan varian (`POST` dan `PATCH`), sinkron dengan `prisma/schema.prisma`.
  - **3. Catalog Dashboard Visibility ([`products/page.tsx`](file:///src/app/dashboard/admin/products/page.tsx) & [`catalog-hierarchy.ts`](file:///src/lib/catalog-hierarchy.ts)):** Menampilkan informasi HPP Modal dan estimasi margin kotor (`+xx%`) secara real-time pada tabel datar (Flat View) dan tabel varian warna (Hierarchy View).
  - **4. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), 34 unit test files dengan 286 unit tests lulus 100% (`pnpm test:unit`).

- **2026-09-22 (Admin Orders Modal Redesign & Financial Reports Real-Time Multi-PT Remediation):**
  - **1. Admin Orders Modal Redesign ([`orders/page.tsx`](file:///src/app/dashboard/admin/orders/page.tsx)):** Perbaikan modal status pesanan dan modal pelacakan kurir live agar konsisten, modern, dan mudah dibaca. Menata ulang kartu modal dengan hierarki visual yang jelas, mencegah teks tertutup/terpotong (`truncate`, `break-words`), mempercantik detail kurir, timeline status, dan tombol aksi dengan transisi halus.
  - **2. Customer & Mobile Orders Refinement ([`orders-client.tsx`](file:///src/app/dashboard/customer/orders/orders-client.tsx), [`mobile-orders-view.tsx`](file:///src/components/customer/mobile-orders-view.tsx), [`live-courier-tracker.tsx`](file:///src/components/shipping/live-courier-tracker.tsx)):** Penyelarasan tampilan status pesanan, integrasi live tracking kurir yang responsif pada desktop dan mobile, serta pemisahan tab status retur yang rapi.
  - **3. Financial Reports Real-Time Multi-PT Engine ([`api/admin/reports/route.ts`](file:///src/app/api/admin/reports/route.ts) & [`export/route.ts`](file:///src/app/api/admin/reports/export/route.ts)):**
    - Pengambilan data 100% real-time dari database PostgreSQL (`Store`, `Order`, `ServiceOrder`, `RentalOrder`, `Complaint`, `ReturnRequest`).
    - Validasi rentang tanggal ketat dengan guard NaN dan fallback ISO UTC string aman.
    - Isolasi akses Multi-PT berbasis `storeId` untuk role `STORE_ADMIN`.
    - Optimasi performa query paralel menggunakan `Promise.all` dan eliminasi bottleneck N+1 query pada ranking toko.
    - Sinkronisasi status omzet (`REVENUE_STATUSES = ['PAID', 'IN_PROGRESS', 'SHIPPED', 'COMPLETED', 'COMPLAINED']`) secara konsisten pada API laporan utama dan ekspor Excel/CSV.
  - **4. Reports UI Modernization ([`reports/page.tsx`](file:///src/app/dashboard/admin/reports/page.tsx)):** Penggantian `window.alert()` dengan notifikasi HUD modern `toast()`, pembersihan tipe `any`, dan proteksi hidrasi.
  - **5. Unit Testing Suite ([`admin-financial-reports.test.ts`](file:///tests/unit/admin-financial-reports.test.ts)):** 7 unit tests lolos 100% memvalidasi kalkulasi metrik finansial, eliminasi dummy fallback, inklusi status omzet, dan integrasi multi-cabang.
  - **6. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), 27 unit test files dengan 255 unit tests lulus 100% (`pnpm test:unit`).

- **2026-09-21 (Chat Toko One-Time Context Popup, Caption Co-Sending & Clean Room Transition):**
  - **1. One-Time Context Banner & Duplicate Suppression ([`chat/page.tsx`](file:///src/app/dashboard/customer/chat/page.tsx)):** Popup floating context kartu pesanan/retur/produk di atas input chat otomatis menghilang seketika saat pesan dikirim (`setActiveOrderContext(null)` dan `setActiveProductContext(null)`). Riwayat ID/nomor pesanan dicatat ke `sentContextsRef` dan URL dibersihkan via `window.history.replaceState` sehingga pop up tidak pernah muncul kembali saat refresh atau navigasi ulang. Jika room obrolan sudah memiliki pesan inkuiri pesanan/retur tersebut, pop up ditekan secara otomatis.
  - **2. Co-Sending Context & User Caption ([`chat/page.tsx`](file:///src/app/dashboard/customer/chat/page.tsx) & [`dashboard/admin/chat/page.tsx`](file:///src/app/dashboard/admin/chat/page.tsx)):** Teks kustom yang diketik pembeli (misal: "Min, tolong cek ya") disatukan langsung ke dalam payload kartu referensi (`note: rawContent`) dalam satu pesan utuh, bukan terpisah menjadi dua pesan. Kartu referensi di sisi customer dan admin secara otomatis menampilkan caption/catatan pengguna dengan kontainer bubble rapi.
  - **3. Unit Testing Suite ([`chat-order-return-context.test.ts`](file:///tests/unit/chat-order-return-context.test.ts)):** 9 unit tests lolos 100% memvalidasi pembuatan parameter chat, parsing pesan referensi terstruktur, penggabungan note/caption pengguna, penekanan banner ganda, dan pemetaan status retur.
  - **4. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), 24 unit test files dengan 232 unit tests lulus 100% (`pnpm test:unit`).

- **2026-09-21 (Real-Time Shipping Engine, Shopee-Calibrated Rates & 40km Gojek Radius Enforcement):**
  - **1. Dedicated Shipping Calculation API ([`api/shipping/calculate/route.ts`](file:///src/app/api/shipping/calculate/route.ts)):** Endpoint POST real-time yang menghitung estimasi jarak berbasis Haversine Formula antara toko asal PT dan alamat pengiriman customer, mengevaluasi radius 40 km Gojek Instant, serta menghitung tarif dinamis JNE (REG / YES) berdasarkan bobot dan zona wilayah Indonesia (Intra-City Jabodetabek Rp 12.000, Inter-Java Rp 19.000, Outer-Java Rp 38.000).
  - **2. Shopee-Calibrated Distance & Rates Engine ([`shipping-engine.ts`](file:///src/lib/shipping/shipping-engine.ts)):** Modul kalkulasi logistik independen dengan dukungan Multi-PT, kalibrasi jarak jalan darat (+25%), pembulatan kilogram per 1.000 gram, validasi server-side anti-tampering (`verifyServerShippingCost`), dan auto-fallback jika koordinat alamat belum terisi.
  - **3. Mobile Shopee Checkout Integration ([`mobile-shopee-checkout-view.tsx`](file:///src/components/checkout/mobile-shopee-checkout-view.tsx)):** Menampilkan pilihan kurir interaktif dengan live badge estimasi jarak (misal `8.5 km`), harga real-time, dan status Gojek Instant (_"Di luar jangkauan (maks 40 km)"_ dengan badge merah disabled saat jarak melebihi 40 km). Dilengkapi sub-toggle JNE REG (2-3 hari) dan JNE YES (1 hari esok sampai) dengan indikator visual modern.
  - **4. Desktop Checkout Integration ([`checkout/page.tsx`](file:///src/app/checkout/page.tsx)):** Menghubungkan perubahan alamat dengan fetch otomatis ke `/api/shipping/calculate`, indikator loading _"Memperbarui tarif API..."_, auto-switch dari Gojek ke JNE Express jika alamat tujuan di luar 40 km, dan proteksi server checkout (`/api/checkout/route.ts`) yang memverifikasi integritas tarif sebelum Midtrans Snap token dibuat.
  - **5. Unit Testing Suite ([`real-time-shipping-engine.test.ts`](file:///tests/unit/real-time-shipping-engine.test.ts)):** 12 unit tests lolos 100% memvalidasi perhitungan jarak Haversine, batasan radius 40 km Gojek Instant, tarif dasar & kilometer lanjutan Gojek, tarif JNE REG & YES (1.75x multiplier), tarif antar-pulau, dan verifikasi anti-tampering server.
  - **6. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), 25 unit test files dengan 244 unit tests lulus 100% (`pnpm test:unit`).

- **2026-09-21 (Customer Mobile Settings Viewport, Top Save Button & Centered Compact Toast Notification HUD):**
  - **1. Sub-View Top Header Action Button ([`mobile-customer-account-view.tsx`](file:///src/components/customer/mobile-customer-account-view.tsx)):** Menyematkan tombol _"Simpan"_ oranye interaktif di bar atas kanan sub-view saat customer mengedit biodata atau mengubah kata sandi, serta tombol _"Tambah"_ pada daftar alamat. Menghilangkan `MobileBottomNav` selama mode sub-view aktif agar tidak bertumpuk dengan keyboard virtual.
  - **2. Virtual Keyboard Pop-up Focus Centering ([`mobile-customer-account-view.tsx`](file:///src/components/customer/mobile-customer-account-view.tsx)):** Mengintegrasikan listener `onFocusCapture` dan resize event `window.visualViewport` yang secara otomatis menggeser/menggulung input form aktif ke tengah layar (`block: 'center'`) dengan transisi mulus saat keyboard virtual perangkat mobile muncul, didukung padding bawah form `pb-80` agar field terbawah tidak tertutup.
  - **3. Dead-Center Compact Pill Toast Notification ([`toast.tsx`](file:///src/components/ui/toast.tsx) & [`toaster.tsx`](file:///src/components/ui/toaster.tsx)):** Memposisikan `ToastViewport` tepat di tengah layar (`fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[999999]`), menata styling menjadi frosted pill gelap yang ramping (`rounded-2xl border px-4 py-2.5 shadow-2xl backdrop-blur-md bg-slate-950/90 text-white`) dengan mikro animasi zoom-in dan fade-in. Memangkas teks pemberitahuan menjadi 1 kalimat pendek dan to-the-point tanpa kata berulang.
  - **4. Unit Testing Suite ([`customer-account-hub.test.ts`](file:///tests/unit/customer-account-hub.test.ts)):** 7 unit tests lolos 100% memvalidasi ketersediaan tombol simpan di header mobile, auto-scroll saat keyboard aktif, penonaktifan navbar bawah pada subview, dan format pesan notifikasi yang ringkas.
  - **5. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), 24 unit test files dengan 230 unit tests lulus 100% (`pnpm test:unit`).

- **2026-09-21 (Chat Toko Order & Return Seamless Context Integration Engine):**
  - **1. Seamless Order & Return Link Payload ([`mobile-orders-view.tsx`](file:///src/components/customer/mobile-orders-view.tsx), [`orders-client.tsx`](file:///src/app/dashboard/customer/orders/orders-client.tsx), [`order-detail-client.tsx`](file:///src/app/dashboard/customer/orders/[orderId]/order-detail-client.tsx)):** Tombol _"Chat Toko"_ pada daftar pesanan mobile, desktop, dan halaman detail pesanan otomatis menyertakan parameter lengkap (`orderId`, `storeId`, `orderNumber`, `returnId`, `returnReason`, `returnStatus`, `returnType`, `productName`, `productImage`, `productPrice`).
  - **2. Contextual Return & Order Header Banner ([`chat/page.tsx`](file:///src/app/dashboard/customer/chat/page.tsx)):** Bilah atas percakapan secara otomatis menampilkan nomor pesanan berserta status retur (_"Retur: Disetujui"_, _"Retur: Menunggu Verifikasi Toko"_, _"Retur: Ditolak"_) dan tautan _"Lihat Rincian"_ yang dapat diklik langsung.
  - **3. Floating Pinned Context Card & Auto-Prefill Message ([`chat/page.tsx`](file:///src/app/dashboard/customer/chat/page.tsx)):** Menampilkan kartu ringkasan interaktif tepat di atas bar input chat dengan thumbnail produk, label ungu _"Pengajuan Retur"_, rincian kendala (_"Kendala: Mati Total (DOA)"_), dan tombol aksi cepat _"Tanya Status Retur"_. Kolom input chat otomatis terisi pesan ramah dengan rincian pesanan dan kendala sehingga customer dapat langsung menekan tombol kirim tanpa mengetik ulang.
  - **4. Unit Testing Suite ([`chat-order-return-context.test.ts`](file:///tests/unit/chat-order-return-context.test.ts)):** 5 unit tests lolos 100% memvalidasi konstruksi parameter URL chat, parsing kendala retur, pembentukan template pesan dinamis, dan pemetaan status retur bahasa Indonesia.
  - **5. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), 24 unit test files dengan 228 unit tests lulus 100% (`pnpm test:unit`).

- **2026-09-21 (Order Return / Dikembalikan Status Tab & Real-Time Tracking Engine):**
  - **1. Dedicated Status Tab ([`mobile-orders-view.tsx`](file:///src/components/customer/mobile-orders-view.tsx) & [`orders-client.tsx`](file:///src/app/dashboard/customer/orders/orders-client.tsx)):** Menambahkan tab status khusus _"Dikembalikan"_ / _"Dikembalikan (Retur)"_ lengkap dengan counter dinamis pada mobile dan desktop, menyelesaikan masalah pesanan yang diajukan retur tidak diketahui lokasinya.
  - **2. Return Order Classification ([`order-return-utils.ts`](file:///src/lib/order-return-utils.ts)):** Fungsi `isReturnOrder` mengelompokkan pesanan dengan status `RETURNED` maupun yang memiliki `returnRequests` aktif ke dalam tab _"Dikembalikan"_, sekaligus memisahkannya dari tab _"Selesai"_ agar riwayat tetap bersih dan jelas.
  - **3. Contextual Real-Time Return Status Badges:** Menampilkan badge status langsung pada kartu pesanan sesuai progres retur (_"Menunggu Verifikasi Retur"_, _"Retur Sedang Ditinjau"_, _"Retur Disetujui"_, _"Pengajuan Retur Ditolak"_, _"Dikembalikan (Retur)"_), serta tombol pintas _"Status Retur"_ untuk melihat perkembangan rincian pengembalian dana/unit.
  - **4. Simplified & Aesthetic Return Status Card ([`order-detail-client.tsx`](file:///src/app/dashboard/customer/orders/[orderId]/order-detail-client.tsx)):** Menata ulang tampilan status retur menjadi jauh lebih simpel, ramping, dan cantik:
    - Menghilangkan badge status ganda yang redundan di kartu hero dan kartu retur.
    - Menambahkan visual 3-step progress stepper (_Diajukan_ ➔ _Verifikasi Toko_ ➔ _Selesai / Refund Cair_).
    - Menata rincian kendala, rekening refund, dan bukti unboxing dalam baris-baris datar (_flat info rows_) tanpa kotak berulang (_box-in-box_).
    - Memperbaiki tipografi nomor pesanan panjang di mobile agar tidak terpotong atau patah baris (_break-all responsive text_).
    - Menyembunyikan tombol ulasan saat produk sedang dalam proses retur agar tidak membingungkan customer.
    - Menyediakan form nomor resi pengiriman balik yang interaktif ketika pengajuan retur disetujui toko.
  - **5. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), 23 unit test files dengan 223 unit tests lulus 100% (`pnpm test:unit`).

- **2026-09-21 (Login WhatsApp 2FA OTP Verification Enforcement Engine):**
  - **1. Multi-Step Login Verification ([`login/page.tsx`](file:///src/app/login/page.tsx)):** Menghubungkan alur login `/login` dengan deteksi 2FA aktif. Akun dengan 2FA aktif otomatis dicegat dan dialihkan ke layar _"Langkah 2: Verifikasi OTP WhatsApp"_. Sesi login tidak akan diberikan sebelum kode OTP 6-digit WhatsApp dimasukkan dengan benar.
  - **2. Dedicated Login 2FA API ([`api/auth/login-2fa/route.ts`](file:///src/app/api/auth/login-2fa/route.ts)):** Endpoint modular untuk mengecek kredensial (`action: 'check'`), men-generate kode OTP login & tautan instan `wa.me`, serta memverifikasi kode OTP (`action: 'verify'`).
  - **3. Persistent Two-Factor Storage Engine ([`two-factor-store.ts`](file:///src/lib/two-factor-store.ts)):** Penyimpanan persisten status 2FA per akun di disk (`.data/two-factor-store.json`) dan memory cache, menjamin konsistensi status 2FA antara dashboard pengaturan dan alur login.
  - **4. Unit Testing Suite ([`login-2fa-flow.test.ts`](file:///tests/unit/login-2fa-flow.test.ts)):** 6 unit tests lolos 100% memvalidasi bypass akun tanpa 2FA, pencegatan akun ber-2FA, validasi masa berlaku OTP 5 menit, batasan 3x salah, dan konsumsi OTP one-time.
  - **5. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), 22 unit test files dengan 218 unit tests lulus 100% (`pnpm test:unit`).

- **2026-09-21 (2FA WhatsApp OTP, Suspicious Login Removal & Accurate Device Sessions Engine):**
  - **1. Suspicious Login Alerts Removal ([`settings/page.tsx`](file:///src/app/dashboard/customer/settings/page.tsx)):** Menghapus total kartu notifikasi login mencurigakan beserta state `loginAlertsEnabled`. Menata ulang grid keamanan menjadi 1 kolom yang bersih dan elegan.
  - **2. Active 2FA WhatsApp OTP Flow ([`security/route.ts`](file:///src/app/api/user/security/route.ts) & [`settings/page.tsx`](file:///src/app/dashboard/customer/settings/page.tsx)):** Mengimplementasikan generator kode OTP 6-digit dengan masa berlaku 5 menit, tautan instan `wa.me` dengan template pesan verifikasi resmi, dan modal verifikasi interaktif via `createPortal`. Mendukung toggle on/off dengan proteksi nomor WhatsApp terdaftar.
  - **3. Accurate Real-Time Device & Session Detection ([`user-agent-parser.ts`](file:///src/lib/user-agent-parser.ts)):** Mengganti mock statis _"MacBook"_ dengan deteksi nyata dari User-Agent (misal: _PC / Desktop (Windows 11 / 10)_ dengan _Google Chrome_, atau _Smartphone (iOS/Android)_). Mendukung pembersihan sesi sekunder nyata via `DELETE /api/user/security` saat tombol _"Keluarkan Perangkat Lain"_ ditekan.
- **2026-09-21 (Admin Financial Reports Real-Time Integrity & Multi-PT Data Engine):**
  - **1. Inclusive Revenue Recognition ([`route.ts`](file:///src/app/api/admin/reports/route.ts) & [`export/route.ts`](file:///src/app/api/admin/reports/export/route.ts)):** Memperluas filter omzet untuk menyertakan status `SHIPPED` dan `COMPLAINED` (`['PAID', 'IN_PROGRESS', 'SHIPPED', 'COMPLETED', 'COMPLAINED']`), menghentikan under-reporting saat pesanan sedang dikirim kurir, dengan tetap mengecualikan pesanan batal (`CANCELLED`) dan retur (`RETURNED`).
  - **2. Active Multi-PT Store Model Migration:** Mengganti query legacy `prisma.mitra` dengan model aktif `Store` (tabel `stores`). Menyajikan data jumlah toko fisik aktif, unit terjual, dan toko rating tertinggi secara akurat dari basis data multi-cabang. Mengganti string hardcoded "· 5 Toko Aktif" pada header omzet dengan nilai dinamis `data.revenue.storeCount`.
  - **3. Active Warranty, Return & Complaint Analytics:** Menggantikan model legacy `Ticket` dengan model aktif `Complaint` dan `ReturnRequest`. Menghitung klaim retur, komplain terselesaikan, dan rata-rata waktu penyelesaian kasus nyata.
  - **4. Zero Dummy Fallbacks & Inverted Label Fix ([`page.tsx`](file:///src/app/dashboard/admin/reports/page.tsx)):** Menghapus total fallback angka dummy (`|| 65`, `|| 25`, `|| 10`) pada diagram donut dan menampilkan empty state elegan saat data kosong. Menyelaraskan label kategori produk dengan dataset (`'Gadget & Sparepart'`, `'Jasa Servis LCD'`, `'Sewa & Aksesoris'`).
  - **5. Hydration Guard & Cache-Control Headers:** Menambahkan HTTP header `Cache-Control: no-store, no-cache, must-revalidate` pada API dan guard `mounted` pada frontend untuk mengeliminasi hydration warning serta menjamin data selalu mutakhir.
  - **6. Unit Testing Suite & Health:** Pembuatan 7 unit test cases baru ([`admin-financial-reports.test.ts`](file:///tests/unit/admin-financial-reports.test.ts)). TypeScript 0 error (`pnpm tsc --noEmit`), 27 unit test files dengan 255 unit tests lulus 100% (`pnpm test:unit`).

- **2026-09-21 (Top-Centered Compact Toast HUD Notification Engine):**
  - **1. Top-Centered Viewport Positioning ([`toast.tsx`](file:///src/components/ui/toast.tsx)):** Memposisikan `ToastViewport` di bagian atas tengah layar (`fixed left-1/2 top-5 -translate-x-1/2 z-[999999] pointer-events-none sm:top-6 sm:max-w-xs`), tidak mepet dan tidak menutupi tengah layar atau keyboard.
  - **2. Compact HUD Pill Design & Top Slide Animation ([`toast.tsx`](file:///src/components/ui/toast.tsx) & [`toaster.tsx`](file:///src/components/ui/toaster.tsx)):** Mengubah tampilan toast menjadi mini pill (`rounded-full px-4 py-2 w-fit max-w-[88vw] backdrop-blur-md bg-slate-950/90 text-white shadow-2xl border border-slate-800/80`) dengan animasi halus meluncur dari atas (`slide-in-from-top-3` dan `slide-out-to-top-2`) dengan durasi 2000ms. Dilengkapi mikro icon (`Check` warna emerald untuk default/success, `AlertCircle` untuk destructive) tanpa tombol 'X'.
  - **3. Simplified Single-Line Copy ([`settings/page.tsx`](file:///src/app/dashboard/customer/settings/page.tsx) & [`address-modal.tsx`](file:///src/components/customer/address-modal.tsx)):** Memangkas teks judul dan deskripsi panjang berulang menjadi 1 kalimat pendek dan to-the-point (misal: "Biodata berhasil disimpan", "Kata sandi berhasil diubah", "Foto profil berhasil disimpan", "Alamat berhasil diperbarui").
  - **4. Unit Testing Suite ([`customer-account-hub.test.ts`](file:///tests/unit/customer-account-hub.test.ts)):** 7 unit tests lolos 100% memverifikasi posisi viewport atas-tengah (`top-5`), styling rounded-full pill, dan copy yang ringkas (<35 karakter).
  - **5. Mobile Header Action Button Consolidation ([`settings/page.tsx`](file:///src/app/dashboard/customer/settings/page.tsx)):** Menghapus tombol "Simpan" redundan di bagian bawah form pada mode mobile (`hidden md:flex` dan `hidden md:inline-flex`), sehingga hanya tombol "Simpan" oranye di header kanan atas yang aktif, menjaga bagian bawah form bersih dan tidak mengganggu keyboard.
  - **6. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), unit test pass 100% (`pnpm vitest run tests/unit/customer-account-hub.test.ts tests/unit/product-review-eligibility.test.ts`).

- **2026-09-21 (Aesthetic & Minimalist Product Share Modal Engine):**
  - **1. Dedicated Share Modal Component ([`product-share-modal.tsx`](file:///src/components/gadget/product-share-modal.tsx)):** Mengembangkan modal berbagi modern yang simpel dan cantik via `createPortal(..., document.body)` (`z-[9999]`, backdrop-blur-xs, mobile bottom-sheet rounded-t-[28px], dan slide-up animation). Dilengkapi mini kartu preview produk (gambar, varian aktif, harga terformat, nama toko fisik), grid 4 kanal sosial cepat dengan branded icon (WhatsApp, Telegram, Facebook, Twitter/X), kotak salin tautan instan 1-klik dengan indikator "Tersalin ✓", dan tombol Web Share API perangkat jika didukung.
  - **2. Mobile Trigger & Bottom Bar Auto-Hide ([`shopee-mobile-product-detail.tsx`](file:///src/components/gadget/shopee-mobile-product-detail.tsx)):** Menghubungkan tombol ikon bagikan (`Share2`) di header mobile ke state `isShareModalOpen`. Mengintegrasikan auto-hide pada bottom floating bar ("Beli Sekarang", "+ Keranjang", "Chat Toko") dan tombol scroll-to-top saat modal share aktif (`!isShareModalOpen`).
  - **3. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), unit test suites pass 100% (`pnpm vitest run tests/unit/product-review-eligibility.test.ts tests/unit/buy-now-direct-checkout.test.ts`).

- **2026-09-21 (Mobile Variant Selection & Programmatic Gallery Scroll Lock Fix):**
  - **1. Programmatic Scroll Guard ([`shopee-mobile-product-detail.tsx`](file:///src/components/gadget/shopee-mobile-product-detail.tsx)):** Menyematkan flag `isProgrammaticScrollRef` dan timer `programmaticTimeoutRef` saat gambar disinkronkan via `useEffect` (akibat klik tombol varian). Mencegah event listener `onScroll` (`handleGalleryScroll`) memicu event scroll balik saat animasi smooth scroll berjalan, yang sebelumnya menyebabkan varian yang baru dipilih ter-reset otomatis kembali ke varian pertama/lama.
  - **2. Touch & Swipe Handlers:** Menambahkan `onTouchStart` dan `onPointerDown` pada gallery container slider agar setiap gestur sentuhan/geseran jari pengguna seketika melepaskan programmatic lock, menjamin navigasi geser manual tetap sinkron 100% dengan pemilihan varian.
  - **3. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), unit test pass 100% (`pnpm vitest run tests/unit/product-review-eligibility.test.ts tests/unit/buy-now-direct-checkout.test.ts`).

- **2026-09-21 (Shopee-Style Mobile Back Button Resilience & Safe-Area Alignment):**
  - **1. Semantic Link & History Fallback Engine ([`shopee-mobile-product-detail.tsx`](file:///src/components/gadget/shopee-mobile-product-detail.tsx)):** Mengonversi tombol back floating mobile menjadi semantic `<Link href="/gadget">` yang diperkuat dengan handler `handleBack`. Memeriksa apakah navigasi berasal dari domain internal dan bukan halaman yang sama (`!document.referrer.endsWith(window.location.pathname)`). Jika ya dan `history.length > 1`, memanggil `window.history.back()` dengan fallback otomatis 200ms ke `router.push('/gadget')` jika navigasi history macet/tidak merespon (seperti pada iframe simulator atau direct URL load).
  - **2. Safe-Area Padding & Stacking Priority:** Menambahkan `pt-[max(0.75rem,env(safe-area-inset-top))]` (dan `pt-[max(0.625rem,env(safe-area-inset-top))]` saat scrolled) pada header floating mobile agar tombol kembali tidak tertutup notch/Dynamic Island/status bar perangkat mobile dan simulator. Menyematkan `z-20`, `cursor-pointer`, dan micro-interaction `active:scale-90` untuk umpan balik sentuhan instan.
  - **3. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), unit test suites pass (`pnpm vitest run tests/unit/product-review-eligibility.test.ts tests/unit/buy-now-direct-checkout.test.ts`).

- **2026-09-21 (Checkout-Completed Review Enforcement Engine):**
  - **1. Checkout-Based Review Eligibility ([`/api/gadgets/[id]/reviews/route.ts`](file:///src/app/api/gadgets/[id]/reviews/route.ts)):** Memperbarui validasi ulasan produk agar tombol ulasan muncul jika customer telah selesai checkout produk tersebut (`status` berada pada rentang checkout valid: `PENDING_PAYMENT`, `PAID`, `IN_PROGRESS`, `SHIPPED`, `COMPLETED`, dan bukan `CANCELLED`). Mengurutkan riwayat berdasarkan `createdAt: 'desc'`. Menolak submit dari customer yang belum checkout produk dengan HTTP 403.
  - **2. Edge & Client Cache Shield:** Menambahkan `export const dynamic = 'force-dynamic'` dan `export const revalidate = 0` pada route ulasan, serta menyematkan `cache: 'no-store'` dan header `Cache-Control: 'no-cache, no-store, must-revalidate'` pada client `fetchReviews` ([`product-reviews-section.tsx`](file:///src/components/gadget/product-reviews-section.tsx)) agar status hak ulasan selalu mutakhir dan anti-stale.
  - **3. UI Guard & Semantic Isolation:** Tombol ulasan ("Tulis Ulasan Pembeli" / "Beri Ulasan Sekarang") hanya muncul untuk produk yang benar-benar telah di-checkout oleh user tersebut. Produk lain yang belum di-checkout otomatis menyembunyikan tombol ulasan dan hanya menampilkan pill informasi: _"Khusus Pembeli yang Selesai Checkout"_.
  - **4. Unit Testing Suite & Database Verification ([`product-review-eligibility.test.ts`](file:///tests/unit/product-review-eligibility.test.ts)):** 7 unit tests lolos 100% menguji status checkout, penolakan non-login, isolasi produk A vs produk B, penolakan order dibatalkan (`CANCELLED`), dan penolakan ulasan tanpa bukti pesanan.
  - **5. Mobile Review Modal Ergonomics & Bottom Bar Auto-Hide ([`shopee-mobile-product-detail.tsx`](file:///src/components/gadget/shopee-mobile-product-detail.tsx), [`product-review-modal.tsx`](file:///src/components/gadget/product-review-modal.tsx)):** Menyembunyikan floating bottom action bar ("Beli Sekarang", "+ Keranjang", "Chat Toko") dan tombol back-to-top secara otomatis saat modal ulasan terbuka (`!isReviewModalOpen`) agar tidak menutupi tombol modal. Memindahkan modal ulasan ke `createPortal(..., document.body)` dengan `z-[9999]` dan body scroll-lock.
  - **6. Safety & Push Guard:** Tidak dilakukan push ke remote repository maupun deploy VPS sesuai instruksi eksplisit user sampai instruksi diberikan.

- **2026-09-20 (Verified Buyer Review Enforcement — Completed Order Requirement):**
  - **1. Strict Backend Review Verification ([`/api/gadgets/[id]/reviews/route.ts`](file:///src/app/api/gadgets/[id]/reviews/route.ts)):** Membatasi hak pembuatan ulasan produk hanya untuk customer terverifikasi yang telah membeli produk tersebut dengan status pesanan `COMPLETED` (`status === 'COMPLETED'`). Menolak percobaan submit dari non-pembeli atau pesanan belum selesai dengan HTTP 403 (`Ulasan hanya dapat diberikan oleh customer yang telah membeli produk ini dan status pesanannya sudah selesai`).
  - **2. Dynamic Eligibility State & UI Guard ([`product-reviews-section.tsx`](file:///src/components/gadget/product-reviews-section.tsx)):** `userEligibility.canReview` hanya bernilai `true` jika customer memiliki riwayat pesanan `COMPLETED` untuk produk terkait atau sudah memiliki ulasan sebelumnya. Menghalangi modal terbuka untuk pengguna yang tidak eligible dengan pesan error yang jelas, menampilkan tombol "Tulis Ulasan Pembeli" khusus bagi pembeli berstatus selesai, dan mengganti tombol kosong menjadi pill informasi: _"Khusus Pembeli dengan Pesanan Selesai"_.
  - **3. Product Review Modal Security Guard ([`product-review-modal.tsx`](file:///src/components/gadget/product-review-modal.tsx)):** Menambahkan blok peringatan verifikasi pesanan dan men-disable tombol submit jika form dibuka tanpa adanya `orderId` terverifikasi atau `existingReview`.
  - **4. Unit Testing Suite ([`product-review-eligibility.test.ts`](file:///tests/unit/product-review-eligibility.test.ts)):** 5 unit tests komprehensif menguji skenario penolakan non-login, penolakan status non-COMPLETED (`PENDING_PAYMENT`, `PAID`, `IN_PROGRESS`, `SHIPPED`, `CANCELLED`), serta persetujuan ulasan bagi pesanan berstatus `COMPLETED`.
  - **5. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), unit test pass 100% (`pnpm vitest run tests/unit/product-review-eligibility.test.ts`).

- **2026-09-18 (Mobile Navigation Standardization, Desktop Footer Isolation & Order Views Polish):**
  - **1. Universal Desktop Footer Isolation ([`footer.tsx`](file:///src/components/layouts/footer.tsx)):** Enforced `hidden md:block` directly on the root `<footer>` element + added optional `className` support. Permanently guarantees that multi-column desktop footer will never leak onto mobile viewports across all pages.
  - **2. Order Confirmation Suite Mobile Nav Integration ([`order-confirmation/*`](file:///src/app/order-confirmation)):** Standardized `MobileTopNav` and `MobileBottomNav` across single order (`[orderId]`), multiple orders (`multiple`), rental (`rental/[orderId]`), sparepart (`sparepart/[orderId]`), and root gateway fallback (`page.tsx`). Fully synchronized loading skeletons, empty states, and order details with clean mobile padding (`pt-4 sm:pt-6 md:pb-24 md:pt-28`).
  - **3. Customer Order Detail & List Navigation Polish ([`order-detail-client.tsx`](file:///src/app/dashboard/customer/orders/[orderId]/order-detail-client.tsx) & [`mobile-orders-view.tsx`](file:///src/components/customer/mobile-orders-view.tsx)):** Eliminated unwrapped footer leak in customer order details, integrated unified `MobileTopNav` (`Pesanan #[orderNumber]`) and `MobileBottomNav` (`currentTab="akun"`), hid redundant desktop breadcrumb on mobile viewports, adjusted sticky filter tabs offset to `top-[61px]`, and replaced dark slate background with clean canvas `bg-slate-50`.
  - **4. Checkout Desktop Footer Leak Fix ([`checkout/page.tsx`](file:///src/app/checkout/page.tsx)):** Wrapped checkout desktop footer in `hidden md:block` to maintain focused, distraction-free checkout experience on mobile.
  - **5. MobileBottomNav Intelligent Routing ([`mobile-bottom-nav.tsx`](file:///src/components/layouts/mobile-bottom-nav.tsx)):** Added detection for `/order-confirmation` routes to set `currentTab="none"`, preventing false active tab highlights.
  - **6. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), 19 unit test suites with 193 unit tests passed 100% (`pnpm test:unit`).

- **2026-09-17 (Mobile-First Modular Experience, Universal Navigation Consistency & Touch Slide Engine):**
  - **1. Universal Mobile Navigation System ([`mobile-top-nav.tsx`](file:///src/components/layouts/mobile-top-nav.tsx) & [`mobile-bottom-nav.tsx`](file:///src/components/layouts/mobile-bottom-nav.tsx)):** Implementasi `MobileTopNav` terpadu dengan tombol back dinamis (`onBack` callback / fallback `router.back()`), search pill interaktif, indikator aktif terisolasi untuk Chat dan Keranjang (ring oranye `ring-2 ring-primary bg-primary/10`), serta counter badge keranjang dinamis riil yang tersinkronisasi dengan Zustand `cart-store` (menghilangkan hardcoded badge '2'). Standarisasi `MobileBottomNav` dengan dukungan tab aktif `'beranda' | 'katalog' | 'toko' | 'chat' | 'akun' | 'none'`, badge counter pesanan/chat dinamis, navigasi akun pembeli langsung ke `/dashboard/customer/settings`, serta opsi `currentTab="none"` untuk membebaskan highlight palsu saat berada di halaman non-tab.
  - **2. Shopee-Style Product Detail Touch Slider & 2-Column Variants ([`shopee-mobile-product-detail.tsx`](file:///src/components/gadget/shopee-mobile-product-detail.tsx)):** Media gallery horizontal slide track natif (`snap-x mandatory touch-pan-x flex overflow-x-auto`) yang mulus dan intuitif tanpa harus menekan tombol panah, dengan tetap mempertahankan tombol panah frosted glass Prev/Next. Transformasi selector varian menjadi layout grid 2-kolom (`grid-cols-2 gap-2.5`) dengan styling clean minimalis. Standardisasi kartu rekomendasi unit lain dan gadget serupa menggunakan `ProductCatalogMiniCard` yang konsisten 100% dengan katalog produk.
  - **3. Customer Chat Interface Polish ([`chat/page.tsx`](file:///src/app/dashboard/customer/chat/page.tsx)):** Integrasi `MobileTopNav` dan `MobileBottomNav` standar pada rute chat customer. Penghapusan search bar berlebih pada layout chat customer untuk memaksimalkan viewport pesan aktif. Indikator aktif dual-layer: ikon Chat di `MobileTopNav` menyala aktif (`isChatActive=true`), sementara `MobileBottomNav` diset `currentTab="none"` agar tab Akun tidak menyala secara keliru.
  - **4. Cart Experience Polish & Dynamic Floating Summary ([`cart/page.tsx`](file:///src/app/cart/page.tsx)):** Menghilangkan teks "Keranjang Belanja" statis berulang di bawah header agar konten langsung rapat dan proporsional dengan navbar atas. Floating Checkout Bar dinamis: bar rincian harga & tombol checkout di bagian bawah hanya muncul saat ada barang yang dicentang (`hasCheckedItems`); otomatis tersembunyi jika tidak ada item yang dipilih. Bottom navigation di halaman keranjang diset `currentTab="none"` tanpa tombol keranjang redundan.
  - **5. Modular Mobile Views Across Core Routes & Desktop Layout Preservation:** Pemisahan bersih antarmuka mobile dan desktop menggunakan arsitektur modular (`MobileHomeView`, `MobileCatalogView`, `MobileStoreDirectoryView`, `MobileStoreProfileView`). Halaman katalog (`/gadget`): Hapus tombol "Beli" pada tiap kartu katalog mobile agar visual rapi dan fokus eksplorasi unit. Pemulihan dan proteksi total layout desktop (`/checkout`, dll) agar tidak terpotong navbar fixed (`pt-28`) atau merusak responsive breakpoint `hidden md:block`.
  - **6. Verification & Health:** TypeScript 0 error (`pnpm tsc --noEmit`), 16 unit test suites lolos (`pnpm test:unit`).

- **2026-09-17 (Midtrans Core API Custom Payment Modal & Zero-Redirect White-Label Engine):**
  - **1. In-House White-Label Payment Modal ([`custom-payment-modal.tsx`](file:///src/components/payment/custom-payment-modal.tsx)):** Menghapus total pengalihan eksternal ke `app.sandbox.midtrans.com` dan pita kuning Sandbox "TEST". Menggantinya dengan modal kustom elegan bertema Slate-50/Trust Blue/Action Orange langsung di website Affiliate Gadget.
  - **2. Local QRIS Base64 Engine ([`qrcode`](file:///src/components/payment/custom-payment-modal.tsx)):** Men-generate gambar QRIS Standar Nasional beresolusi tinggi langsung dari `qr_string` resmi Midtrans menggunakan library `qrcode` (Base64 Data URL). Bebas kendala CORS/CSP Next.js, 100% offline-resilient, scannable di seluruh mobile banking dan e-wallet, dilengkapi tombol "Unduh Gambar QR".
  - **3. Core API Charge Engine & 406 Conflict Shield ([`midtrans.ts`](file:///src/lib/midtrans.ts) & [`/api/payment/charge`](file:///src/app/api/payment/charge/route.ts)):** Mengimplementasikan Midtrans Core API langsung untuk kanal QRIS, BCA VA, Mandiri Bill, BNI VA, dan BRI VA. Mencegah error HTTP 406 (state conflict) dengan menyertakan tag kanal dan attempt ID unik per charge (`orderNumber-METHOD-XXXX`), serta mencatat ID aktif di `payment.notes`.
  - **4. Background Status Polling & Webhook Reconciliation ([`/api/payment/status`](file:///src/app/api/payment/status/route.ts) & [`/api/payment/midtrans/webhook`](file:///src/app/api/payment/midtrans/webhook/route.ts)):** Polling background otomatis setiap 3,5 detik yang mengubah pesanan jadi `PAID` dan pembayaran jadi `VERIFIED` saat settlement. Webhook mendukung pencocokan base order number multi-segmen.
  - **5. Clean Variant Selector Styling ([`shopee-mobile-product-detail.tsx`](file:///src/components/gadget/shopee-mobile-product-detail.tsx) & [`/gadget/[id]/page.tsx`](file:///src/app/gadget/[id]/page.tsx)):** Menghapus background gelap/norak pada selector varian agar tampilan bersih, terang, dan minimalis.
  - **6. Verification & Health:** 18 test suites dengan 189 unit tests lulus 100% (`pnpm test:unit`) dan TypeScript 0 error (`pnpm tsc --noEmit`).
...
- **2026-09-17 (Direct Buy Now Engine — Zero Cart Pollution & Instant Checkout Architecture):**
  ...
  - **3. Smart Isolated Checkout Pipeline ([`checkout/page.tsx`](file:///src/app/checkout/page.tsx)):** Mendeteksi mode direct buy (`isDirectBuy`), membatasi checkout hanya pada item yang baru dibeli tanpa mencampurkan item lain yang sedang tersimpan di keranjang pembeli. Menyediakan navigasi "Kembali ke Produk" dan membersihkan direct item setelah pesanan dibuat tanpa menghapus isi keranjang reguler.
  - **4. Comprehensive Unit Test Suite ([`buy-now-direct-checkout.test.ts`](file:///tests/unit/buy-now-direct-checkout.test.ts)):** 17 test suites dengan 184 unit tests lulus 100% (`pnpm test:unit`) dan TypeScript 0 error (`pnpm tsc --noEmit`).
...
- **2026-09-16 (Refined Luxury Mobile Product Detail Page & Zero-Dummy Real Data Engine):**
  ...
  - **3. Integrated Mobile Experience:** Menyertakan stepper kuantitas pesanan (`-` `qty` `+`), kartu jaminan paket bonus 3-in-1 berdesain minimalis elegan, selector varian berbingkai bersih, dan floating bottom action bar ergonomis (`Chat Toko`, `+ Keranjang`, `Beli Sekarang`).
...
- **2026-09-16 (4-Level Catalog Hierarchy & Bulk Color Variant Update Engine):**
  ...
  - **3. Atomic Bulk Color Update API ([`/api/admin/products/variants/bulk-color-update`](file:///src/app/api/admin/products/variants/bulk-color-update/route.ts)):** Eksekusi perubahan massal harga & stok varian warna terpilih dalam transaksi atomik Prisma `$transaction`, rekalkulasi otomatis harga terendah (`Product.price`) dan total stok produk induk (`Product.stock`), serta pencatatan audit log keamanan platform.
  - **4. Native 4-Level Dropdown Catalog Page ([`page.tsx`](file:///src/app/dashboard/admin/products/page.tsx)):** Transformasi penuh katalog superadmin menjadi tampilan hirarki dropdown bertingkat native di halaman utama (`Merek` -> `Seri` -> `RAM/Storage` -> `Varian Warna`), normalisasi cerdas seri Samsung/Apple, inline quick price updater 1-klik per kapasitas tanpa modal popup, multi-select sticky bar, dan toggle mode hirarki vs tabel datar.
  - **5. Verification & Test Suite ([`bulk-color-variant-update.test.ts`](file:///tests/unit/bulk-color-variant-update.test.ts)):** 16 test suites dengan 180 unit tests lolos 100% (`pnpm test:unit`) dan TypeScript 0 error (`pnpm tsc --noEmit`).
...
- **2026-09-08 (Checkout Security, Architecture Hardening, Product-to-Store Direct Chat & Real-Time Sync: Penutupan Kerentanan Checkout, Idempotency Redis, Fitur Chat Toko Langsung dari Produk, dan Sinkronisasi Multi-Produk):**
  ...
  - **3. Checkout Hardening, UI Polish & Idempotency (Sprint 3):** Penomoran pesanan ber-entropi tinggi anti-tabrakan (`PREFIX-YYYYMMDD-8HEX`) dengan auto-retry Prisma P2002, standardisasi `CHECKOUT_ERROR_CODES` & `CheckoutError`, guard produk tanpa toko (`PRODUK_TANPA_TOKO`), evaluasi dinamis bonus 3-in-1, sub-toggle kurir JNE REG vs YES, eliminasi 100% tipe `any`, dan proteksi double-submit via Redis Idempotency Key (`X-Idempotency-Key`).
  - **4. Product-to-Store Chat Integration (Sprint 4):** Penambahan relasi langsung `storeId` pada model `AdminChatRoom`, endpoint API `POST /api/customer/chat/store-room` untuk pembuatan/pembukaan kamar chat toko langsung tanpa order, kartu referensi produk otomatis atomik (`product_reference`), tombol interaktif "Chat Toko" di detail produk dengan proteksi sesi & context parameters, auto-open conversation di `/dashboard/customer/chat` dengan Next.js 15 Suspense boundary, serta preview sidebar & judul toko cabang mandiri.
  - **5. Real-Time Chat Optimization & Multi-Product Consistency (Sprint 5):** Pencegahan refresh/wipe pesan optimistik saat kirim pesan via `isSendingRef`, pembaruan algoritma pengecekan duplikasi `productId` di backend `store-room` agar pertanyaan multi-produk pada toko yang sama memunculkan card bubble berurutan, sinkronisasi URL via `router.replace` tanpa desinkronisasi Next.js App Router, siklus hidup `loadedRoomIdRef`, dan tombol aksi "Chat Toko" terpadu di seluruh section produk landing page.
  - **6. Verification & Test Suite:** 100% lolos kompilasi TypeScript (`pnpm tsc --noEmit` = 0 error) dan 8 test suites dengan 86 unit tests lolos 100% (`pnpm test:unit`).

- **2026-09-07 (Authentication Security Foundation & Infrastructure Harmonization: Implementasi Keamanan Login & Register, Unit Testing Vitest, dan Unifikasi Docker Production):**
  - **1. Backend Register Hardening (Tahap 1):** Server-side Zod validation, sanitasi & normalisasi email, password policy kuat (8+ karakter, upper, lower, number, symbol), honeypot anti-bot field, rate limiting dual-layer per-IP (5x/jam) & per-email (3x/jam) via Redis sliding window, dan password hashing bcrypt cost factor 12.
  - **2. Backend Login Security (Tahap 2):** Refactoring `authorize()` callback NextAuth dengan rate limit per IP (10x/15m) & per email (5x/15m), lockout akun otomatis setelah 5 kali gagal (15m cooldown), progressive delay hingga 5 detik, timing attack anti-enumeration via `DUMMY_HASH` compare, serta pengecekan status akun aktif (`isActive`).
  - **3. Standarisasi Error Codes & DoS Protection:** Implementasi `LOGIN_ERROR_CODES` constant dan pembatasan panjang password `max(128)` untuk menolak payload CPU ReDoS/bcrypt DoS.
  - **4. Unit Testing Suite (Vitest):** Pembuatan 38 unit test cases (`rate-limit.test.ts`, `login-security.test.ts`, `auth-validation.test.ts`, `security-crypto.test.ts`) dengan mocking `server-only` untuk pengujian otomatis 100% pass.
  - **5. Frontend Register UX Upgrade ([`/register`](file:///src/app/register/page.tsx)):** Live interactive password strength checklist, indikator centang real-time 5 kriteria, input nomor HP opsional, dan pencegahan double-submit.
  - **6. Unifikasi Docker Single Stack & Repository Cleanup:** Konsolidasi seluruh environment ke dalam satu [`docker-compose.yml`](file:///docker-compose.yml) tunggal standar untuk lokal dan VPS hosting, serta pembersihan ~48 MB dump database usang, skrip debug usang, dan file installer hardcoded.

- **2026-08-31 (GitHub Repository Initialization & Main Branch Push: https://github.com/Kizker/affiliate-gadget):**
  - **1. Inisialisasi Repository Baru GitHub:** Membuat repository baru di akun GitHub `Kizker` bernama `affiliate-gadget` secara otomatis via GitHub REST API dengan visibilitas _public_.
...
- **2026-08-30 (Second-Hand Gadget Marketplace Transformation: Transformasi Total Seluruh Informasi Platform Menjadi Penjualan Gadget Second Berkualitas):**
  - **1. Standarisasi Kondisi Unit Second:** Menerapkan standarisasi kondisi fisik unit gadget second di seluruh katalog publik, landing page, dan admin CMS: `LIKE_NEW` (_Second Like New - Mulus 99%_), `SECOND_MULUS` (_Second Mulus - 95% s/d 98%_), dan `GRADE_A` (_Second Grade A - Normal 100%_).
  ...
  - **3. Transformasi Landing Page & Showcase:** Pembaruan hero headline (_Beli Gadget Second Lebih Tenang & Terjamin_), 3D Coverflow cards, pilar nilai (_Trust Pillars_), dan tabel komparasi nilai (_Value Props_) yang membandingkan Affiliate Gadget terhadap penjual second online tanpa jaminan/uji teknisi.
  - **4. Penyelarasan Transaksi & Post-Purchase:** Mengadaptasi halaman keranjang, checkout, konfirmasi pesanan, modal pengajuan komplain garansi toko (_Tukar Unit Teruji_), dan modal pengajuan retur (_Tukar Unit Pengganti_).
...
- **2026-08-30 (Buyer Reviews & Media Attachments Architecture: Sistem Ulasan Pembeli dengan Lampiran Foto & Video [/gadget/[id]]):**
  ...
  - **2. Senior UI/UX Redesign — Unified 2-Column Rating Overview:** Mengeliminasi elemen redundan (kapsul `100% Asli`, tombol edit di header, kotak ketiga pilar garansi sekunder, dan subtitle berulang). Merestrukturisasi ringkasan penilaian menjadi tata letak **2-Kolom Terpadu dengan Whitespace Optimal**:
    - _Kolom Kiri (Hero Rating Score)_: Skor besar tegas (`4.8 / 5.0`), 5 bintang emas, total ulasan pembeli, dan chip rekomendasi kepuasan (`98% Pembeli Merekomendasikan`).
    - _Kolom Kanan (Interactive Rating Bars)_: Bar chart filter distribusi bintang (5★ s/d 1★) yang responsif dengan divider vertikal halus dan visual feedback saat diklik.
  - **3. Standarisasi Luxury Portal Lightbox (Harmonisasi dengan Modul Klaim Garansi):** Mengadopsi arsitektur modal Lightbox layar penuh (`createPortal` ke `document.body`) dengan glassmorphism backdrop (`bg-white/45 backdrop-blur-xl`), tombol navigasi melingkar hitam solid (`bg-black/80 hover:bg-black text-white`), panggung media `rounded-3xl` dengan border halus, filmstrip thumbnail `bg-white/80 border border-slate-200/80 backdrop-blur-xl`, chip nama reviewer & rating bintang terpusat di tengah atas (_centered floating pill_), eliminasi tombol silang X demi tampilan bersih (menutup cukup dengan klik area luar/ESC), dan keyboard shortcut helper persis seperti pada modul Klaim Garansi.
  - **4. Senior UI/UX Redesign — Minimalist Filter & Sort Toolbar:** Mengeliminasi pembungkus border ganda (_box-in-a-box_) dan menghapus tombol bintang bernilai nol (`⭐ 3 (0)`, `⭐ 2 (0)`, `⭐ 1 (0)`). Toolbar kini menggunakan **Pill Kapsul Segmented Mandiri** (_Semua_, _Dengan Foto & Video_, _5 Bintang_, _4 Bintang_), ikon bintang amber SVG solid tanpa emoji, serta dropdown pengurutan ramping (_Paling Baru, Rating Tertinggi, dll_) yang terintegrasi secara proporsional.
  ...
  - **12. Senior UI/UX Polish — Clean Justified Left-Right Footer Layout ([`footer.tsx`](file:///Users/andrichadeamitra/Documents/PROJECT/affiliate-gadget/src/components/layouts/footer.tsx)):** Menghapus kolom cabang fisik dan menata ulang layout menjadi **Rata Kanan-Kiri Terpadu (_Justify Between Layout_)**:
    - _Sisi Kiri (Left Anchor)_: Identitas brand, logo, deskripsi, dan badge verifikasi jaringan resmi.
    - _Sisi Kanan (Right Anchor)_: Kolom navigasi _Menu Utama_ dan _Merek Populer_ yang merapat rapi ke tepi kanan dengan spasi horizontal proporsional dan transisi hover Action Orange.
    - _Bawah_: Hak cipta di sisi kiri dan link ketentuan/kontak di sisi kanan. Menghilangkan ruang kosong asimetris dan menghasilkan footer yang seimbang dan lapang.
  ...
  - **14. Return & Refund Architecture (Fitur Pengajuan Pengembalian pada Pesanan Selesai [/dashboard/customer/orders & /orders/[orderId]]):**
    - _Skema Database & Backend_: Menambahkan model `ReturnRequest` beserta enum `ReturnType` (_REFUND_ vs _REPLACEMENT_) dan `ReturnStatus` (_PENDING_, _IN_REVIEW_, _APPROVED_, _REJECTED_, _COMPLETED_), sinkronisasi PostgreSQL via Prisma DB push, serta endpoint API [`/api/returns`](file:///Users/andrichadeamitra/Documents/PROJECT/affiliate-gadget/src/app/api/returns/route.ts) dan [`/api/returns/[id]`](file:///Users/andrichadeamitra/Documents/PROJECT/affiliate-gadget/src/app/api/returns/%5Bid%5D/route.ts).
    - _Komponen Modal Interaktif ([`return-modal.tsx`](file:///Users/andrichadeamitra/Documents/PROJECT/affiliate-gadget/src/components/customer/return-modal.tsx))_: Dialog modern minimalis dengan selektor solusi (_Refund 100%_ vs _Tukar Unit Baru_), preset kendala cepat (_Cacat Fisik_, _Mati Total_, _Salah Varian_, dll), form rekening pengembalian dana, upload multi-media bukti unboxing, validasi 30 hari, dan tombol konfirmasi Action Orange.
    - _Integrasi UI/UX Pesanan_: Tombol **`Ajukan Pengembalian`** pada kartu pesanan `COMPLETED` di daftar pesanan ([`orders-client.tsx`](file:///Users/andrichadeamitra/Documents/PROJECT/affiliate-gadget/src/app/dashboard/customer/orders/orders-client.tsx)) dan **Kartu Live Status Pengembalian (Bento Section)** pada halaman detail pesanan ([`order-detail-client.tsx`](file:///Users/andrichadeamitra/Documents/PROJECT/affiliate-gadget/src/app/dashboard/customer/orders/%5BorderId%5D/order-detail-client.tsx)).
  - **15. Senior UI/UX Redesign — Return & Refund Modal ([`return-modal.tsx`](file:///Users/andrichadeamitra/Documents/PROJECT/affiliate-gadget/src/components/customer/return-modal.tsx)):**
    - _Eliminasi Truncation & Sticky Layout_: Mengadopsi arsitektur modal 3-lapisan terstruktur (**Fixed Sticky Header**, **Smooth Scrollable Body**, **Fixed Sticky Footer**), sehingga tombol konfirmasi pengajuan selalu tampil utuh di layar tanpa terpotong batas bawah viewport browser.
    - _Penyederhanaan Teks & Subtitle_: Menghapus subtitle berulang di header dan label petunjuk redundan.
    - _Harmonisasi Warna & Bento Cards_: Mengeliminasi warna border biru yang bertabrakan, menggantinya dengan **Modern Segmented Solution Cards** bernuansa _Action Orange & Neutral Slate_ dengan indikator centang di sudut atas dan chip nilai (_100%_ & _Unit Baru_).
    - _Minimalist Focus & Micro-interactions_: Textarea kendala dengan focus ring oranye lembut, galeri media unboxing yang proporsional, serta tombol aksi _Action Orange Pill_ kontras tinggi.
  - **16. Senior UI/UX Redesign — Admin Returns & Refund Hub ([`/dashboard/admin/returns`](file:///Users/andrichadeamitra/Documents/PROJECT/affiliate-gadget/src/app/dashboard/admin/returns/page.tsx) & [`sidebar.tsx`](file:///Users/andrichadeamitra/Documents/PROJECT/affiliate-gadget/src/components/dashboard/sidebar.tsx)):**
    - _Eliminasi Header Redundan & Maksimalkan Whitespace_: Menghapus blok header judul & subtitle berulang agar halaman langsung fokus pada **Bento Metrics** dan **Daftar Pengajuan** (konsisten dengan halaman Produk, Pesanan, dan Klaim Garansi).
    - _Single-Row Unified Control Panel (Identik dengan Katalog Gadget & Pesanan)_: Menata ulang toolbar menjadi 1 baris terpadu berarsitektur `rounded-3xl` dengan Segmented Pills Status (`Semua Status`, `Perlu Verifikasi`, `Sedang Diperiksa`, `Disetujui`, `Selesai`, `Ditolak`) di sisi kiri, serta Dropdown Solusi, Filter Cabang Toko, Input Pencarian dengan tombol hapus (X), dan tombol Refresh instan di sisi kanan.
    - _Harmonisasi Bento Metrics (Anti-Rainbow Palette)_: Mengeliminasi 5 warna pastel bertabrakan (kuning, biru, hijau, oranye, abu) yang menimbulkan kebisingan visual (_visual noise_). Menyatukannya menjadi **4 Kartu Bento Mewah & Bersih** berlatar putih/gelap netral dengan tipografi mono tebal tabular, indikator denyut amber (_pulse_) untuk antrean baru, dan penghapusan subtitle mikro berulang.
    - _2-Column Spacious Bento Card_: Merombak 3 kotak abu-abu bertumpuk menjadi **Tata Letak 2-Kolom Terpadu**:
      - _Kolom Kiri_: Strip info unit gadget dengan thumbnail beresolusi tajam, harga pesanan, tag kendala, dan galeri foto/video unboxing ber-aspect squircle halus.
      - _Kolom Kanan_: Rincian rekening bank tujuan refund atau alur tukar unit tersegel, tanggapan resmi toko cabang, dan nomor resi pengiriman balik.
    - _Action Hub High Contrast_: Tombol aksi _Action Orange Pill_ kontras tinggi (_Setujui Pengajuan_, _Mulai Pemeriksaan Unit_, _Konfirmasi Selesai_, _Beri Tanggapan_) yang responsif dan sangat jelas.
    - _Luxury Portal Media Lightbox_: Viewer foto dan video unboxing layar penuh berbasis portal dengan keyboard navigation (ESC, panah kiri/kanan) dan filmstrip thumbnail.
...
- **2026-08-30 (User Profile Photo Integration: Tampilan Foto Profil Pengguna di Navbar & Dashboard Sidebar [/ & /dashboard]):**
  ...
  - **3. Integrasi Foto Profil di Sidebar Dashboard:** Menghubungkan foto profil pengguna di bagian footer sidebar dashboard ([`sidebar.tsx`](file:///Users/andrichadeamitra/Documents/PROJECT/affiliate-gadget/src/components/dashboard/sidebar.tsx)) baik pada mode diperluas maupun mode mini _collapsed_.
...
- **2026-08-29 (In-App Chat Toko Architecture: Penggantian Link Eksternal WhatsApp Menjadi Chat Toko Internal [/toko, /toko/[slug], & /gadget/[id]]):**
  - **1. Integrasi Chat Toko Internal:** Mengganti seluruh tombol dan aksi _"Chat Sales WA"_ / _"Hubungi WhatsApp"_ menjadi **"Chat Toko"** yang langsung terhubung ke modul chat internal platform ([`/dashboard/customer/chat`](file:///Users/andrichadeamitra/Documents/PROJECT/affiliate-gadget/src/app/dashboard/customer/chat/page.tsx)), sehingga pengguna tidak lagi diarahkan keluar ke aplikasi pihak ketiga (WhatsApp).
  - **2. Standarisasi Ikon & Styling:** Mengganti ikon telepon `PhoneCall` menjadi ikon pesan `MessageSquare`, dengan aksen warna _Action Orange_ (`text-orange-500` / `bg-orange-500`) yang selaras dengan tema modern minimalis platform.
...
- **2026-08-29 (Senior UI/UX Redesign: Modern Minimalist Product Card Architecture [/gadget, /toko, & Homepage]):**
  ...
  - **2. Perbaikan Truncation & Layout Metadata:** Menyelaraskan nama toko cabang dan pill status stok (`Stok: X Unit` / `Sisa X!`) dengan spasi proporsional tanpa terpotong (_no awkward truncation_).
  - **3. Formasi Harga Solid & Anti-Wrap:** Memperbaiki layout harga sehingga nominal `Rp X.XXX.XXX` tampil dalam format satu baris yang tebal, tegas, dan kontras tinggi tanpa pemecahan teks (_anti-wrap glitch_).
  - **4. Solid Action Orange CTA Button:** Menggunakan tombol _Action Orange Pill_ (`bg-orange-500 hover:bg-orange-600 active:scale-[0.98] text-white shadow-sm shadow-orange-500/25 font-bold`) yang mencolok dan meningkatkan konversi klik, lengkap dengan micro-animation icon panah.
...
- **2026-08-29 (Product Inventory UI/UX: Tampilan Total Stok & Rincian Stok Per Varian [/gadget & /gadget/[id]]):**
  ...
  - **2. Rincian Stok Per Varian Hardware:** Setiap opsi varian RAM/Storage menampilkan badge ketersediaan stok spesifik (`Stok: X`, `Sisa X!`, atau `Habis`), serta indikator stok terpilih yang langsung tersinkronisasi dengan stepper kuantitas dan proteksi stok habis (_out-of-stock guard_).
...
- **2026-08-27 (Role-Adaptive Settings Architecture: Pengaturan Khusus Admin Platform [/dashboard/admin/settings]):**
  - **1. Pemisahan Hak Akses Profil Toko:** Admin Platform (`ADMIN`) dan Superadmin (`SUPER_ADMIN`) tidak lagi dibebani formulir _Profil Toko & PT Cabang_ fisik, melainkan langsung difokuskan pada **Profil Akun Pengelola** dan **Keamanan & Password**.
  ...
  - **3. Action Orange Branding & Bento Layout:** Tombol simpan perubahan _Action Orange Pill_, avatar monogram squircle, dan kartu ringkasan cakupan hak akses platform.
...
- **2026-08-27 (Senior UI/UX Redesign & Simplified Naming: Admin Platform Dashboard View [/dashboard/admin]):**
  ...
  - **2. Modern Linear List Layout:** Menghapus kotak abu-abu tebal bersarang pada kartu Daftar Toko dan menggantinya dengan divider baris halus, avatar squircle monogram oranye hangat (`R`, `W`, `B`, `P`, `J`), hover oranye, serta chip status semantik (_● Terverifikasi_).
  - **3. Balanced 4-Card Action Grid:** Mengubah 2 kotak vertikal yang canggung pada kartu Aksi Cepat menjadi grid 4 kartu pintasan proporsional (_Katalog Gadget, Daftar Toko, Klaim Garansi, Pengaturan_) dengan ikon berwarna, deskripsi ringkas, dan animasi panah interaktif.
...
- **2026-08-27 (Senior UI/UX Harmonization: Action Orange Branding pada Dashboard Utama [/dashboard/admin]):**
  ...
  - **2. Proportional Top Branch Highlight:** Cabang peringkat 01 (Roxy Mas Pusat) pada Distribusi Omzet Toko kini memiliki squircle rank oranye (`bg-orange-100/90 text-orange-700`) dan progress bar _Action Orange_ (`bg-orange-500 shadow-2xs shadow-orange-500/20`).
...
- **2026-08-27 (Senior UI/UX Harmonization: Action Orange Branding pada Daftar Toko [/dashboard/admin/mitras]):**
  - **1. Primary CTA (+ Tambah Toko):** Mengganti tombol hitam menjadi _Action Orange Pill_ (`bg-orange-500 hover:bg-orange-600 active:scale-95 text-white shadow-sm shadow-orange-500/25 font-bold`).
...
- **2026-08-27 (Senior UI/UX Redesign: Manajemen Pengguna Table List View [/dashboard/admin/users]):**
  ...
  - **3. Clean Data Table List Layout:** Menggantikan representasi 3-kolom kartu lebar yang boros scroll dengan format **Data Table List** berstandar Linear & Apple:
    - _Kolom 1 (Pengguna & Profil)_: Avatar squircle monogram oranye lembut / foto profil, Nama tebal dengan hover orange, dan alamat email.
    - _Kolom 2 (Peran & Akses)_: Chip semantik peran (Super Admin - purple, Admin Platform - blue, Admin Toko - indigo, Customer - slate, Teknisi - orange).
    - _Kolom 3 (Kontak & Telepon)_: Nomor telepon / WA berformat monospace rapi.
    - _Kolom 4 (Status Akun)_: Status pill semantik (_● Aktif_ / _● Nonaktif_).
    - _Kolom 5 (Terdaftar)_: Format tanggal registrasi bahasa Indonesia ringkas (_26 Agu 2026_).
    - _Kolom 6 (Aksi Cepat)_: Aksi toggle aktif/nonaktif cepat, tombol Edit Role, dan hapus akun.
  - **4. Redesain Modal Bento Modern (`CreateUserModal` & `EditUserModal`):** Dialog berarsitektur Bento modern dengan backdrop blur lembut, input field ramping `rounded-xl` dengan focus orange, squircle header icon oranye, dan tombol aksi utama _Action Orange Pill_ (`bg-orange-500 hover:bg-orange-600 text-white`).

- **2026-08-27 (Senior UI/UX Polish: Eliminasi Header Judul & Relokasi Filter Periode [/dashboard/admin/reports]):**
  - **1. Zero Top Header Noise:** Menghapus blok judul _"Laporan Finansial"_ dan teks subtitle deskripsi di bagian atas agar 4 kartu KPI omzet langsung tampil di layar tanpa pemborosan ruang vertikal.
...
- **2026-08-27 (Senior UI/UX Master Redesign: 2-Column Responsive Layout [/mitras/[id]/edit]):**
  ...
  - **2. Top Floating Control Bar:** Menambahkan bar kontrol atas yang ramping dengan tombol _← Kembali_, Breadcrumb Toko, tautan _Halaman Publik ↗_, dan tombol utama **Simpan Perubahan** (_Solid Black Pill_), sehingga pengguna dapat langsung menyimpan tanpa harus scroll jauh ke bawah.
...
- **2026-08-27 (Senior UI/UX Polish: Eliminasi Header Judul & Back Link [/mitras/[id]/edit]):**
  - **1. Zero Top Header Noise:** Menghapus blok judul _"Edit Profil Cabang Toko"_, subtitle deskripsi, dan link kembali di bagian atas agar kartu Bento formulir langsung tampil bersih dan efisien di layar tanpa pemborosan ruang vertikal.

- **2026-08-27 (Penyelarasan Lebar Kontainer & Responsivitas Sidebar [/mitras/[id]/edit & /mitras/create]):**
  - **1. Sinkronisasi Grid & Header Alignment:** Mengubah kontainer halaman formulir dari `max-w-4xl` menjadi `max-w-6xl mx-auto` agar sejajar presisi dengan header atas (_Lihat Toko Publik_ dan toggle sidebar) serta daftar tabel master toko.
  - **2. Dynamic Sidebar Adaptation:** Kontainer kini otomatis meregang (_fluid stretch_) dan terpusat rapi saat sidebar dibuka maupun ditutup (_collapsed/expanded_), persis seperti halaman Dashboard, Pesanan, dan Katalog Produk.

- **2026-08-27 (Senior UI/UX Redesign: Bento Surface Cards pada Formulir Edit Toko [/mitras/[id]/edit/page.tsx]):**
  - **1. Eliminasi Nested Gray Box:** Menghapus kotak abu-abu tebal (_nested container_) dan menggantinya dengan kartu permukaan Bento terpisah berlatar putih bersih (`rounded-2xl border border-slate-200/80 bg-white p-5 sm:p-6 shadow-2xs`).
  ...
  - **3. Standardisasi Input & Tombol Aksi:** Input field seragam berukuran `rounded-xl` dengan bayangan halus `shadow-2xs` serta sticky action footer dengan tombol _Solid Black Pill_ (`Simpan Perubahan`).
...
- **2026-08-27 (Standardisasi Terminologi Global: Penggantian Istilah "Mitra" Menjadi "Toko"):**
  - **1. Standarisasi Teks & Label Antarmuka:** Mengubah seluruh sebutan _"Mitra"_ di modul admin, form tambah/edit cabang, halaman kelola pengguna, dan laporan finansial menjadi terminologi **"Toko"** / **"Cabang Toko"** / **"Admin Toko"**.
  - **2. Toast & Validasi Feedback:** Menyelaraskan pesan sukses dan notifikasi error (misal: _"Data toko berhasil diperbarui!"_, _"Cabang toko berhasil ditambahkan!"_, dan _"Status akun toko berhasil diperbarui"_).
...
- **2026-08-27 (Senior UI/UX & Data Architecture: Pendataan Lengkap Toko & Modal Detail [/dashboard/admin/mitras]):**
  - **1. Integrasi Pendataan Lengkap Toko (Operational, Financial & Legal):**
    ...
    - **Kolom 5 (Status):** Status operasional semantik (_● Terverifikasi_ / _● Pending_).
    - **Kolom 6 (Aksi Terpadu):** Tombol **Detail 🔍**, **Publik ↗**, **Tangguhkan / Setujui**, **Edit**, dan **Hapus**.
  - **2. Modal Detail Pendataan Lengkap Toko:** Membuka dialog terstruktur saat mengklik tombol _Detail_ yang menyajikan:
...
- **2026-08-27 (Senior UI/UX Polish: Eliminasi Header Redundan & Relokasi Tombol Tambah Toko [/dashboard/admin/mitras]):**
  - **1. Zero Top Header Noise:** Menghapus blok judul _"Daftar Toko"_, chip cabang, dan deskripsi subtitle atas agar 4 kartu metrik KPI langsung terlihat di layar tanpa pemborosan ruang vertikal.
...
- **2026-08-27 (Senior UI/UX Redesign: Daftar Toko Table List View [/dashboard/admin/mitras]):**
  - **1. Clean Data Table List Layout:** Mengubah representasi kartu toko 3-kolom menjadi format **Data Table List** yang rapi, kompak, dan konsisten seperti halaman katalog master gadget dan manajemen pengguna:
    - Kolom 1 (_Toko & Cabang_): Monogram avatar squircle, nama toko tebal, dan chip kota.
    - Kolom 2 (_Alamat Fisik_): Alamat lengkap toko dengan ikon gedung, pas terpotong rapi (_single-line truncate_).
    - Kolom 3 (_Kontak & WA_): Nomor telepon / WhatsApp monospace.
    - Kolom 4 (_Status_): Status pill semantik (_Terverifikasi / Pending_).
    - Kolom 5 (_Aksi_): Tombol aksi terpadu (_Lihat Publik ↗_, _Tangguhkan / Setujui_, _Edit_, dan _Hapus_).
  - **2. Bento Metric Cards & Filter Toolbar:** 4 kartu metrik proporsional di bagian atas bersama segmented capsule switcher (_Semua Toko_, _Terverifikasi_, _Menunggu Review_), dropdown kota, dan live search box.
...
- **2026-08-27 (Senior UI/UX Polish: Ukuran Font Nominal Proporsional & Zero Truncation [/dashboard/admin]):**
  - **1. Proportional Nominal Typography:** Mengganti font monospace besar yang terpotong (_"458.900.0..."_) menjadi tipografi sans-serif proporsional modern (`text-lg sm:text-xl font-bold font-sans tabular-nums`) dengan label mata uang `Rp` (12px `text-xs font-semibold text-slate-400`), pas 100% dalam kontainer tanpa ada angka yang terpotong elipsis.
...
- **2026-08-27 (Pembersihan Total Header Atas Dashboard [/dashboard/admin]):**
  - **1. Zero Top Bar Noise:** Menghapus dropdown filter cabang toko dan tombol _+ Tambah Gadget_ dari header atas, sehingga halaman dashboard langsung menampilkan kartu metrik KPI dan insight bento secara bersih tanpa header bar yang mengambang.

- **2026-08-27 (Pembersihan Header Dashboard: Eliminasi Judul & Subtitle Redundan [/dashboard/admin]):**
  - **1. Ultra-Minimal Top Header:** Menghapus blok judul _"Ringkasan Toko"_, role badge, dan teks subtitle deskripsi di bagian atas halaman dashboard. Kontrol aksi (Dropdown filter cabang toko dan tombol _+ Tambah Gadget_) kini langsung berada rapi di pojok kanan atas, sehingga 4 kartu KPI langsung terlihat di layar tanpa pemborosan ruang vertikal.

- **2026-08-27 (Penyederhanaan Navigasi Sidebar: Label "Dashboard" Tunggal [/components/dashboard/sidebar.tsx]):**
  - **1. Clean Navigation Label:** Mengubah label navigasi menu utama dari _"Dashboard Toko"_ dan _"Dashboard Platform"_ menjadi cukup berlabel **"Dashboard"** untuk seluruh tingkatan role (`SUPER_ADMIN`, `STORE_ADMIN`, dan `ADMIN`), memberikan navigasi yang lebih ringkas, universal, dan elegan.

- **2026-08-27 (Senior UI/UX Polish: Bento Metric Alignment & Zero Collision [/dashboard/admin]):**
  - **1. Zero Line-Break Currency Formatting:** Menghilangkan pemenggalan baris canggung pada angka metrik (_"Rp \n 458.900.000"_ dan _"5 Toko \n Aktif"_), menggantinya dengan format mata uang proporsional satu baris (`Rp` label 12px + nominal `text-xl sm:text-2xl font-bold font-mono tracking-tight`) yang tidak akan pernah terpotong pada berbagai resolusi layar.
  - **2. Per-Store Proportional Distribution Progress Bars:** Mengganti bar segmented tipis di atas kontainer menjadi _individual progress bars_ per baris cabang toko (Roxy Mas 53.5%, WTC Surabaya 28.0%, BEC Bandung 18.5%) dengan visualisasi pangsa pasar yang intuitif.
  ...
  - **4. Global Multi-PT Sanitizer pada Profil Avatar:** Memastikan nama profil pengguna pada avatar bawah sidebar (`session.user.name`) otomatis membersihkan istilah _"Multi-PT"_ menjadi label murni _"Super Admin"_.

- **2026-08-27 (Penghapusan Seluruh Istilah "Multi-PT" pada Antarmuka Dashboard & Sidebar):**
  - **1. Standardisasi Terminologi Bersih:** Mengubah seluruh sebutan _"Multi-PT"_ menjadi label yang lebih lugas dan profesional: _"Dashboard Multi-PT"_ ➔ **"Dashboard Toko"**, _"Laporan Finansial Multi-PT"_ ➔ **"Laporan Finansial"**, _"Daftar Toko & PT"_ ➔ **"Daftar Toko"**, _"Ringkasan Multi-PT"_ ➔ **"Ringkasan Toko"**, dan _"5 Toko PT"_ ➔ **"5 Toko Aktif"**.
  - **2. Footer & Profil Pengguna:** Memperbarui footer admin menjadi _"© 2026 Affiliate Gadget • Platform Toko Resmi Indonesia"_ dan menghapus embel-embel _"(Cabang PT)"_ pada info akun pengguna.
...
- **2026-08-27 (Fitur Lengkap Profil Pembeli Ala Shopee & Multi-Alamat [/dashboard/customer/settings]):**
  - **1. Tab 1 - Profil & Biodata Pembeli Komprehensif (Shopee Standard):**
    - Input Nama Lengkap sesuai identitas, Username unik (`@username`), Email (dengan chip `Terverifikasi`), Nomor Telepon/WhatsApp aktif kurir, Pilihan Jenis Kelamin (_Laki-laki / Perempuan_), Tanggal Lahir (dengan benefit voucher ulang tahun), dan Bio/Catatan khusus pengiriman.
  ...
  - **2. Tab 2 - Manajemen Multi-Alamat Pengiriman & 4-Level Wilayah Indonesia (Shopee-Grade):**
    ...
    - **Format Title Case Otomatis:** Mengonversi data wilayah huruf kapital menjadi teks _Title Case_ bersih dan profesional.
    ...
    - **True Fullscreen Portal Lightbox (`createPortal` & `z-[99999]`):** Memindahkan rendering modal langsung ke `document.body`, melepaskan modal dari batasan _CSS transform stacking context_ kontainer `<main>` sehingga efek latar belakang blur (`backdrop-blur-md bg-slate-950/60`) menutupi 100% seluruh layar secara penuh dari sudut paling atas tanpa celah.
    - **Integrasi Peta Interaktif & OpenStreetMap Fallback ([`address-map-picker.tsx`](file:///Users/andrichadeamitra/Documents/PROJECT/affiliate-gadget/src/components/maps/address-map-picker.tsx)):** Menghilangkan kotak error API key Google Maps lama, menggantinya dengan preview peta visual OpenStreetMap interaktif, tombol _"Deteksi GPS Saya"_ (`navigator.geolocation`), dan chip koordinat presisi driver Gojek/JNE.
    - **Penyederhanaan Label & Header Bersih:** Menghapus subtitle sekunder (_"Format resmi wilayah Kemendagri & kurir ekspedisi JNE / Gojek"_) dan label _"Titik Peta GPS"_ pada kolom kanan untuk membuat tampilan modal semakin minimalis, lapang, dan langsung fokus ke interaksi pengguna.
    - **Minimalist Inline Checkbox:** Mengubah kotak toggle alamat utama yang tebal menjadi checkbox inline modern dan ringan (_"Atur sebagai alamat utama"_), mengeliminasi kontainer abu-abu dan icon sparkle yang berlebihan.
    - **Sticky Action Footer:** Tombol CTA _"Simpan Alamat"_ (Solid Black Pill) dan _"Batal"_ selalu terlihat di bagian bawah modal tanpa terpengaruh scroll viewport.
    - **Penandaan Semantik:** Badge `Alamat Utama` (Default), chip `Titik GPS Terpasang`, dan kontrol 1-klik untuk _"Atur Sebagai Utama"_, _"Ubah"_, dan _"Hapus"_.
  - **3. Tab 3 - Suite Keamanan Akun:**
    ...
    - Pemantauan Sesi & Perangkat Login Aktif (Desktop/MacBook vs Mobile iPhone) dilengkapi tombol _"Keluarkan Perangkat Lain"_.
...
- **2026-08-26 (Penyederhanaan Terminologi: Standardisasi Label "Toko" [/dashboard/customer/orders/[orderId]]):**
  - **1. Label Consistency:** Mengubah semua label _"Cabang Toko"_ dan _"Toko Cabang"_ menjadi cukup berlabel **"Toko"** pada seluruh komponen (Header Pill, Strip Logistik Terpadu, Garansi Toko, Pesan Chat Toko, dan Form Pembatalan), memberikan konsistensi visual dan teks yang ringkas bagi pembeli.
- **2026-08-26 (Senior UI/UX Redesign Kartu Ringkasan Pembayaran & Garansi Trust Strip [/dashboard/customer/orders/[orderId]]):**
  - **1. Visual Harmony & Status Header:** Menempatkan status pembayaran semantik (_"Lunas"_ emerald pill atau _"Menunggu Bayar"_ amber pill) langsung di baris header atas sejajar dengan judul kartu, menyederhanakan hierarki informasi.
  ...
  - **3. Eliminasi "Nested Gray Box Trap":** Mengubah kotak abu-abu garansi 30 hari yang bertumpuk menjadi strip garansi resmi terpadu (_Official Store Guarantee Strip_) ber-ikon squircle emerald lembut dan tipografi 11px yang sangat nyaman dibaca.
- **2026-08-26 (Senior UI/UX Redesign Rincian Unit Gadget, Logistik Terpadu & Ringkasan Pembayaran [/dashboard/customer/orders/[orderId]]):**
  ...
  - **2. Integrasi Logistik Terpadu (Zero Separate Box Trap):** Mengeliminasi kartu raksasa logistik terpisah di bawah halaman. Informasi Toko Cabang Pemroses dan Kurir JNE/Asuransi kini terintegrasi secara elegan dalam satu strip bawah kartu produk (_single cohesive surface_).
...
- **2026-08-26 (Penyederhanaan UI: Eliminasi Kartu Bantuan Sekunder [/dashboard/customer/orders/[orderId]]):**
  - **1. Zero Secondary Clutter:** Menghapus kotak pintasan bantuan (_"Butuh Bantuan Pesanan?"_) di kolom kanan rincian pesanan agar tampilan halaman lebih ringkas, padat, dan langsung terfokus pada ringkasan transaksi serta garansi 30 hari resmi.
- **2026-08-26 (Senior UI/UX Redesign Bento Status Klaim Garansi 30 Hari [/dashboard/customer/orders/[orderId]]):**
  - **1. Eliminasi "Box-in-Box Trap" & 2-Column Balanced Bento:** Mengubah kontainer berkotak-kotak menjadi layout 2-kolom seimbang: Sisi Kiri menampilkan _Kendala yang Dilaporkan_ dan Sisi Kanan menyajikan _Hasil Resolusi & Tindakan Toko_ secara berdampingan tanpa nested border yang bertumpuk.
...
- **2026-08-26 (Senior UI/UX Redesign Modal Rating & Ulasan [rating-modal.tsx]):**
  - **1. Zero-Clutter Header & Verified Badge:** Menampilkan judul 18px font-black (_"Beri Ulasan Gadget"_), badge verifikasi amber lembut (_"Terverifikasi"_), dan nomor order monospace yang rapi.
  - **2. Interactive Star Rating Center & Dynamic Emotion Labels:** Bintang rating interaktif dengan efek hover pembesaran halus (`hover:scale-120`), warna emas hangat (`amber-400`), dan label sentimen dinamis semantik (_"1: Sangat Kecewa"_, _"3: Cukup Baik"_, _"5: Luar Biasa / Sempurna"_).
  - **3. Quick Feedback Tag Chips:** Menambahkan pintasan chip ulasan 1-klik (_⚡ Pengiriman Cepat, 📦 Packing Aman & Asuransi, 📱 Unit 100% Original, 🎁 Bonus 3-in-1 Lengkap, 💬 Respon Toko Ramah_) untuk mempercepat dan mempermudah pengisian ulasan oleh pembeli.
  - **4. Input Ergonomis & High-Contrast CTA:** Textarea abu-abu netral lembut (`rounded-2xl border border-slate-200/80 bg-slate-50/60`), tombol _Batal_ rounded-full, serta tombol aksi _Simpan Ulasan_ solid black pill yang tegas dan responsif.
- **2026-08-26 (Senior UI/UX Redesign Modal Pengajuan Klaim Garansi 30 Hari [complaint-modal.tsx]):**
  - **1. Zero-Clutter Header & Crisp Badging:** Mengganti ikon oranye raksasa yang memenuhi ruang dengan header minimalis modern: judul berukuran 18px font-black, badge _Ganti Baru_ emerald lembut, dan nomor order monospace yang rapi.
  - **2. Segmented Grid Chips Kategori Kendala:** Mengatur pilihan kendala umum (_Layar & LCD, Baterai & Daya, Kamera & Audio, Mati Total / Mesin, Lainnya_) ke dalam grid 2-3 kolom rapi dengan active state kontras tinggi (_Sleek Black Pill_), mengeliminasi tata letak tombol bertumpuk yang berantakan.
  ...
  - **4. Upload Area Ringkas & High-Contrast CTA:** Dropzone upload bukti foto/video dibuat lebih ramping dengan thumbnail squircle ber-badge status play video, tombol _Batal_ rounded-full, dan tombol _Ajukan Klaim_ solid black pill kontras tinggi yang tegas.
- **2026-08-26 (Senior UI/UX Redesign Halaman Rincian Pesanan [/dashboard/customer/orders/[orderId]]):**
  - **1. Eliminasi Carnival Buttons & In-Platform Chat Focus:** Mengganti deretan tombol warna-warni yang bertabrakan dengan hierarki aksi terfokus:
    ...
    - Sisi Kanan: Aksi utama workflow yang kontras tinggi (_"Bayar Sekarang"_ Solid Orange untuk pending, _"Konfirmasi Pesanan Diterima"_ Solid Emerald untuk dikirim, _"Klaim Garansi 30 Hari"_ Sleek Black Pill, dan _"Beri Ulasan"_ subtle pill).
...
- **2026-08-26 (Senior UI/UX Redesign Halaman Pesanan Saya [/dashboard/customer/orders]):**
  - **1. Unified Control Panel (Sesuai Desain Manajemen Produk):**
    - Mengadopsi kontainer _Single-Surface Control Panel_ (`rounded-3xl border border-slate-200/80 bg-white p-2.5 sm:p-3 shadow-2xs`) yang identik 100% dengan modul Manajemen Produk.
    ...
    - Kapsul pencarian responsif (`lg:w-72`) dengan pembersih 1-klik (_Clear button_ `X`).
    ...
    - Desain kartu pesanan mengadopsi _Single-Surface Bento Card_ (`rounded-3xl border border-slate-200/70 bg-white hover:border-slate-300 shadow-2xs hover:shadow-xs`) dengan hierarki informasi yang sangat lapang dan teratur.
  ...
  - **2. Hierarki Visual Tinggi & Redesign Kartu Bento:**
    - **Single Cohesive Bento Surface:** Menghilangkan pemisah bertumpuk (_three-decker sandwich_). Kartu pesanan kini berupa satu kanvas `rounded-3xl` lapang dengan padding proporsional (`p-5 sm:p-6`).
    - **Thumbnail Lebih Besar & Jernih:** Ukuran squircle gadget ditingkatkan menjadi `80x80px` - `96x96px` dengan proteksi fallback jernih.
    - **Pengelompokan Keuangan & Aksi yang Ergonomis:** Total tagihan (`font-mono text-2xl font-black`) berdampingan langsung dengan tombol aksi _Chat Toko_ dan tombol utama _Bayar Sekarang / Rincian Pesanan_, mengeliminasi ruang kosong di tengah kartu.
    - Nomor pesanan monospace `#SPR-...` dilengkapi tombol salin 1-klik (_Copy to clipboard_) dengan umpan balik instan.
    - Status badge bergradasi semantik (_Menunggu Pembayaran_, _Diproses Toko_, _Sedang Dikirim_, _Selesai & Diterima_) dengan live status dot.
