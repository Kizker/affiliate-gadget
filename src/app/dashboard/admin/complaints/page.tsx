...
export default function AdminComplaintsPage() {
  ...
  const [filterType, setFilterType] = useState<'ALL' | 'SERVICE' | 'PRODUCT'>(
    'ALL'
  )
  ...
  const filteredComplaints = useMemo(() => {
    return complaints.filter((c) => {
      ...
      const isService = c.order.technicianId !== null
      const matchesType =
        filterType === 'ALL' ||
        (filterType === 'SERVICE' && isService) ||
        (filterType === 'PRODUCT' && !isService)
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="rounded-2xl bg-gradient-to-r from-red-600 via-orange-600 to-yellow-600 p-6 text-white shadow-lg">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold">Manajemen Komplain</h1>
              <span className="flex items-center justify-center rounded-full bg-white/20 px-3 py-1 text-sm font-medium backdrop-blur-sm">
                {complaints.length} Total
              </span>
            </div>
            <p className="mt-1 text-red-100">
              Kelola komplain pelanggan untuk order produk dan jasa servis
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Filters & Search */}
      <div className="sticky top-20 z-10 -mx-4 border-y border-gray-200 bg-white/80 px-4 py-4 backdrop-blur supports-[backdrop-filter]:bg-white/60 sm:top-0 sm:mx-0 sm:rounded-2xl sm:border sm:shadow-sm">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex gap-2 rounded-xl bg-gray-100 p-1">
              <button
                ...
                className={`flex items-center gap-2 rounded-lg px-3 py-1.5 text-sm font-medium transition-all ${filterType === 'ALL' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-2 rounded-lg px-3 py-1.5 text-sm font-medium transition-all ${filterType === 'PRODUCT' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-2 rounded-lg px-3 py-1.5 text-sm font-medium transition-all ${filterType === 'SERVICE' ? 'bg-white text-orange-600 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
              >
            ...
            <div className="relative w-full sm:w-72">
              ...
              <input
                type="text"
                placeholder="Cari order, nama, subject..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full rounded-xl border border-gray-200 bg-gray-50 py-2 pl-10 pr-4 text-sm outline-none transition-all focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10"
              />
            </div>
          </div>
        ...
        {orderIds.length === 0 ? (
          ...
          <div className="space-y-6">
            {orderIds.map((orderId) => {
              ...
              const isService = complaint.order.technicianId !== null
              const typeLabel = isService ? 'Jasa Servis' : 'Produk/Sparepart'
              const TypeIcon = isService ? Briefcase : ShoppingBag
              ...
              return (
                <motion.div
                  ...
                  exit={{ opacity: 0, scale: 0.95 }}
                  className="group overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-gray-100 transition-all hover:shadow-lg hover:ring-blue-100"
                >
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                      <div className="space-y-1">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="flex items-center gap-1 rounded-full bg-white px-2 py-0.5 text-xs font-medium text-gray-600 ring-1 ring-gray-200">
                            <TypeIcon className="h-3 w-3" />
                            {typeLabel}
                          </span>
                          {/* Complaint Navigation Counter */}
                          {totalComplaints > 1 && (
                  ...
                  <div className="grid gap-8 p-6 sm:grid-cols-12 sm:px-8">
                    ...
                    <div className="space-y-6 sm:col-span-8">
                          ...
                          <div className="flex gap-3 overflow-x-auto pb-2">
                            {complaint.images.map((img, idx) => (
                              <button
                                ...
                                className="group/img relative aspect-square w-24 flex-shrink-0 overflow-hidden rounded-xl border border-gray-100 shadow-sm transition-all hover:w-28 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                              >
                      ...
                      {/* Action Area - OPEN state: Start handling (auto-assigned to this user) */}
                      {complaint.status === 'OPEN' && (
                        <div className="pt-2">
                          <button
                            ...
                            className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-6 py-3 font-semibold text-white shadow-lg shadow-blue-200 transition-all hover:-translate-y-0.5 hover:bg-blue-700 hover:shadow-xl disabled:opacity-50 sm:w-auto"
                          >
                            {actionLoading &&
                      ...
                      {complaint.status === 'IN_PROGRESS' && isAssignedToMe && (
                        <div className="rounded-2xl bg-blue-50/50 p-4 ring-1 ring-blue-100/50">
                          {!showRejectForm ? (
                            <div className="space-y-4">
                              <div>
                                ...
                                <div className="relative">
                                  <textarea
                                    ...
                                    rows={3}
                                    className="w-full rounded-xl border border-gray-200 py-3 pl-4 pr-4 shadow-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10"
                                  />
                                </div>
                              </div>
                    ...
                    {/* Right Column: Customer & Item Info Card */}
                      ...
                      <div className="rounded-2xl bg-gray-50 p-5 ring-1 ring-gray-200">
                        ...
                        <div className="space-y-3">
                          ...
                          <div>
                            ...
                            <span
                              className={`mt-1 inline-flex items-center rounded-lg px-2.5 py-1 text-xs font-medium ${isService ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}
                            >
                              {isService ? 'Jasa Servis' : 'Produk / Sparepart'}
                            </span>
                          </div>
                          <div>
                            <p className="text-xs text-gray-500">Total Order</p>
                            <p className="font-bold text-gray-900">
                              Rp {complaint.order.total.toLocaleString('id-ID')}
                            </p>
                          </div>
                        </div>
                      </div>
