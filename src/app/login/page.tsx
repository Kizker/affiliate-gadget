...
import { Eye, EyeOff, Mail, Lock, Loader2, ArrowRight, ShieldCheck } from 'lucide-react'
...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let redirectUrl = (redirectParam && redirectParam.startsWith('/')) ? redirectParam : '/'
      let retries = 3
        ...
        try {
          ...
          if (userRes.ok) {
            ...
            if (adminStaffRoles.includes(userData.role)) {
              ...
              redirectUrl = userData.mitraStatus === 'PENDING' ? '/dashboard/mitra/pending' : '/dashboard/mitra'
  ...
  const handleGoogleSignIn = async () => {
    setIsLoading(true)
    await signIn('google', { callbackUrl: redirectParam || '/api/auth/redirect' })
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100 flex flex-col justify-between relative selection:bg-orange-500 selection:text-white">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[350px] bg-gradient-to-b from-orange-100/30 via-slate-100/20 to-transparent blur-3xl dark:from-slate-800/20" />
      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <Link
                    href="/hubungi-kami"
                    className="text-[11px] font-semibold text-slate-500 hover:text-slate-950 dark:text-slate-400 dark:hover:text-white transition-colors"
                  >
                    Bantuan Sandi?
                  </Link>
                </div>
              ...
              <div className="pt-2">
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full inline-flex items-center justify-center gap-2 rounded-full bg-slate-950 py-3 text-xs font-bold text-white shadow-xs hover:bg-slate-800 active:scale-98 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed dark:bg-blue-600 dark:hover:bg-blue-700"
                >
                  {isLoading ? (
                    <>
