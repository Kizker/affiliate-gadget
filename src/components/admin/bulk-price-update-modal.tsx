...
export function BulkPriceUpdateModal({
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      ...
      toast.success('File template & data katalog berhasil diunduh!')
    } catch (err: any) {
    ...
    <Dialog open={isOpen} onOpenChange={handleModalClose}>
      <DialogContent className="max-w-xl rounded-3xl border border-slate-200/80 bg-white/95 p-6 shadow-2xl backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/95 sm:p-7">
        <DialogTitle className="flex items-center gap-2.5 text-base font-bold text-slate-950 dark:text-white">
          <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-emerald-500/10 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400">
            <FileSpreadsheet className="h-5 w-5" />
          </div>
          <span>Update Harga & Stok Massal via Excel (SKU System)</span>
        </DialogTitle>

        <DialogDescription className="text-xs text-slate-500 dark:text-slate-400">
          Perbarui harga jual, harga coret, dan stok inventori katalog seluruh
          toko secara otomatis melalui file spreadsheet.
        </DialogDescription>

        {!result ? (
          <div className="mt-4 space-y-5">
            {/* Download Template Shortcut Banner */}
            <div className="flex flex-col items-start justify-between gap-3 rounded-2xl border border-blue-100 bg-blue-50/60 p-4 dark:border-blue-900/40 dark:bg-blue-950/30 sm:flex-row sm:items-center">
              <div className="space-y-0.5">
                <span className="text-xs font-bold text-blue-950 dark:text-blue-200">
                  1. Unduh Data & Template Excel
                </span>
                <p className="text-[11px] leading-relaxed text-blue-700/80 dark:text-blue-300/70">
                  Ekspor seluruh katalog produk dan varian yang ada beserta SKU
                  masing-masing.
                </p>
              </div>
              <button
                ...
                className="inline-flex shrink-0 items-center gap-1.5 rounded-xl bg-blue-600 px-3.5 py-2 text-xs font-bold text-white shadow-sm shadow-blue-600/25 transition hover:bg-blue-700 active:scale-95 disabled:opacity-60"
              >
                {downloadingTemplate ? (
                  <>
            ...
            <div className="space-y-2">
              <span className="text-xs font-bold text-slate-900 dark:text-white">
                2. Unggah Berkas Excel Hasil Perubahan
              </span>
