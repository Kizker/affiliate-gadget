...
const RATING_FEEDBACK: Record<
  number,
  { text: string; color: string; desc: string }
> = {
  1: {
    text: 'Sangat Kecewa',
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        if (!isImage && !isVideo) {
          toast.error(
            `File "${file.name}" tidak didukung. Unggah foto atau video saja.`
          )
          continue
        }
        ...
        if (isVideo && videos.length >= 2) {
          toast.error(
            'Maksimal 2 video unboxing/ulasan yang dapat dilampirkan.'
          )
          continue
        }
      ...
      <div className="no-scrollbar relative max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-3xl border border-slate-200/80 bg-white p-6 shadow-2xl duration-200 animate-in fade-in zoom-in-95 dark:border-slate-800 dark:bg-slate-900 sm:p-7">
        ...
        <div className="flex items-start justify-between border-b border-slate-100 pb-4 dark:border-slate-800">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base font-bold tracking-tight text-slate-950 dark:text-white sm:text-lg">
                {existingReview
                  ? 'Perbarui Ulasan Gadget'
                  : 'Tulis Ulasan & Komentar Pembeli'}
              </h2>
              <span className="inline-flex items-center gap-1 rounded-full border border-emerald-200/50 bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">
          ...
          <button
            type="button"
            onClick={onClose}
            className="shrink-0 cursor-pointer rounded-full p-2 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-800 dark:hover:text-white"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-5">
          {!orderId && (
            <div className="flex items-start gap-2.5 rounded-2xl border border-amber-200/80 bg-amber-50 p-3.5 text-xs text-amber-800 dark:border-amber-900/60 dark:bg-amber-950/40 dark:text-amber-200">
              <ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-amber-600 dark:text-amber-400" />
              <div>
                <p className="font-bold">Verifikasi Pesanan Diperlukan</p>
                <p className="mt-0.5 text-[11px] leading-relaxed text-amber-700 dark:text-amber-300">
                  Ulasan hanya dapat dikirim oleh customer yang telah membeli
                  barang ini dan status pesanannya telah selesai (Diterima).
                </p>
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-100 bg-slate-50/70 p-4 py-2 text-center dark:border-slate-800 dark:bg-slate-800/40">
            ...
            <div className="mb-2 flex items-center gap-1.5 sm:gap-2">
              {[1, 2, 3, 4, 5].map((star) => (
                <button
                  ...
                  className="cursor-pointer p-1 transition-all duration-150 hover:scale-125 active:scale-95"
                  title={`${star} Bintang`}
                >
                  <Star
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full resize-none rounded-2xl border border-slate-200/80 bg-slate-50/60 p-3.5 text-xs font-medium leading-relaxed text-slate-900 outline-none transition focus:border-slate-900 focus:bg-white dark:border-slate-800 dark:bg-slate-800/50 dark:text-white"
            ...
            <div className="grid grid-cols-3 gap-2.5 sm:grid-cols-4">
              ...
              {images.map((url, idx) => (
                ...
                >
                  ...
                  />
                  <div className="absolute left-1 top-1 flex items-center gap-1 rounded-md bg-slate-950/60 px-1.5 py-0.5 text-[9px] font-bold text-white">
                  ...
                  <button
                    ...
                    className="absolute right-1 top-1 rounded-full bg-rose-600 p-1 text-white opacity-90 shadow-sm transition hover:opacity-100"
                    title="Hapus foto"
                  >
              ...
              {videos.map((url, idx) => (
                <div
                  key={`vid-${idx}`}
                  className="group relative flex aspect-square items-center justify-center overflow-hidden rounded-2xl border border-orange-200 bg-slate-950 dark:border-orange-800"
                >
                  <video
                    src={url}
                  ...
                  <div className="absolute left-1 top-1 flex items-center gap-1 rounded-md bg-orange-600 px-1.5 py-0.5 text-[9px] font-bold text-white">
              ...
              {(images.length < 5 || videos.length < 2) && (
                <button
                  ...
                  onClick={() => fileInputRef.current?.click()}
                  disabled={uploading}
                  className="flex aspect-square cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/50 p-2 text-slate-400 transition hover:border-orange-500 hover:bg-orange-50/30 hover:text-orange-600 dark:border-slate-800 dark:bg-slate-800/40 dark:hover:border-orange-500"
                >
                  {uploading ? (
                    <div className="flex flex-col items-center gap-1 text-[10px] font-bold text-orange-500">
          ...
          <div className="flex items-center justify-end gap-2.5 border-t border-slate-100 pt-3 dark:border-slate-800">
            ...
            <button
              type="submit"
              disabled={submitting || uploading || rating === 0 || !orderId}
              className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-full bg-orange-500 px-7 py-2.5 text-xs font-bold text-white shadow-sm shadow-orange-500/25 transition hover:bg-orange-600 active:scale-95 disabled:opacity-40"
            >
              {submitting ? (
                <>
